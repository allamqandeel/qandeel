...
describe('R1-01 — the offered set follows the reader\'s own situation', () => {
  ...
  it('a historical reader whose live focus is disclosed here is offered exactly the four that apply', () => {
    ...
    expect(offeredIds(orientationModel(store, projectionFor(store, fetched(TWO_CONTEXT_WORLD()))))).toEqual([
      'RETURN_LIVE_HEAD',
      'RETURN_LIVE_FOCUS',
      'RETURN_WORLD',
      'GO_LIVE_AND_LOCATE',
  ...
  it('the composite is offered only where it is materially distinct from its own halves', () => {
    ...
    expect(offeredIds(orientationModel(historical, projectionFor(historical, fetched(TWO_CONTEXT_WORLD()))))).toContain('GO_LIVE_AND_LOCATE');
...
describe('R1-02 — six meanings remain six, offered or not', () => {
  it('every frozen identity keeps its own effect, promises and wording', () => {
    ...
    expect(new Set(meanings.map((meaning) => meaning.label)).size).toBe(6);
    expect(new Set(meanings.map((meaning) => meaning.hint)).size).toBe(6);

    // The two opposites stay opposite: Live Head promises time and not a place; Live Focus promises
    // a place and not time.
    expect({ time: returnMeaning('RETURN_LIVE_HEAD').movesTime, camera: returnMeaning('RETURN_LIVE_HEAD').movesCamera }).toEqual({ time: true, camera: false });
    expect({ time: returnMeaning('RETURN_LIVE_FOCUS').movesTime, camera: returnMeaning('RETURN_LIVE_FOCUS').movesCamera }).toEqual({ time: false, camera: true });
  ...
  it('no generic identity exists to stand in for any of them', () => {
    for (const id of RETURN_OPPORTUNITY_IDS) {
      expect(/^(home|reset|navigate|go ?live|back or home|return)$/iu.test(returnMeaning(id).label.trim())).toBe(false);
    }
