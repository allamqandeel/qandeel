...
describe('RuntimeEventPublisher',()=>{
 const savedRedis=process.env.REDIS_URL;afterEach(()=>{if(savedRedis===undefined)delete process.env.REDIS_URL;else process.env.REDIS_URL=savedRedis;});
 ...
 it.each([
  ['missing service-role database configuration',false,'redis://configured','available','not_configured'],
  ['missing Redis URL',true,undefined,'available','not_configured'],
  ['configured but disconnected transport',true,'redis://configured','degraded','degraded'],
  ['configured and connected transport',true,'redis://configured','available','available'],
 ]as const)('reports capability for %s without connecting or publishing',(_name,databaseEnabled,redisUrl,transportState,expected)=>{if(redisUrl)process.env.REDIS_URL=redisUrl;else delete process.env.REDIS_URL;const repository={enabled:databaseEnabled}as RuntimeEventAdminRepository,transport={readinessStatus:transportState,connect:jest.fn(),publish:jest.fn(),close:jest.fn()}as unknown as RuntimeEventTransport,publisher=new RuntimeEventPublisher(repository,transport,{recordPublisherOperation:jest.fn()}as unknown as TelemetryService);expect(publisher.readinessStatus).toBe(expected);expect(transport.connect).not.toHaveBeenCalled();expect(transport.publish).not.toHaveBeenCalled();});
