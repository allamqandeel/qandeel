...
export class HimDefinitionRegistry {
  ...
  private validate(d:HimMetricDefinition):void{
    ...
    if(!HIM_OWNERS.includes(d.hifOwner)||!HIM_CALCULATION_STATUSES.includes(d.calculationStatus)||!HIM_SEMANTIC_MAPPING_STATUSES.includes(d.semanticMappingStatus))throw new BadRequestException('Unsupported canonical metadata.');
    if((d.semanticMappingStatus==='RESOLVED'&&(d.semanticType===null||!HIM_SEMANTIC_TYPES.includes(d.semanticType)))||(d.semanticMappingStatus==='UNRESOLVED'&&d.semanticType!==null))throw new BadRequestException('Invalid semantic mapping state.');
