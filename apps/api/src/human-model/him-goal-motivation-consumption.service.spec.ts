...
const OTHER_GOAL = '00000000-0000-4000-8000-000000000009';
const BINDING = '00000000-0000-4000-8000-000000000004';
const OTHER_BINDING = '00000000-0000-4000-8000-000000000005';

const unboundRow = (): HimGoalMotivationSourceRow => ({
  binding_state: 'NO_ACTIVE_GOAL',
...
} as unknown as HimGoalMotivationSourceRow);

const boundRow = (numericValue: number | null): HimGoalMotivationSourceRow => ({
  binding_state: 'ACTIVE_GOAL_BOUND',
  binding_context_id: GOAL,
  slot_order: 1,
  metric_key: 'hse.motivation',
  definition_version: 1,
  hif_owner: 'HSE',
  semantic_mapping_status: 'RESOLVED',
  semantic_type: 'STATE',
  calculation_status: 'CALIBRATED',
  // The canonical persisted definition of hse.motivation@1 is measurement-valid
  // in BOTH contexts. QHIA-010 activates the GOAL context only; the presence of
  // SITUATION here is exactly why the consumer must pin the exact context kind
  // rather than trusting definition eligibility.
  valid_context_kinds: ['SITUATION', 'GOAL'],
  context_kind: 'GOAL',
  context_id: GOAL,
  has_canonical_current_value: numericValue !== null,
  source_metric_key: numericValue === null ? null : 'hse.motivation',
  ...
  source_context_kind: numericValue === null ? null : 'GOAL',
  source_context_id: numericValue === null ? null : GOAL,
...
const service = (rows: unknown) => {
  const repository = { readSessionGoalMotivation: jest.fn().mockResolvedValue(rows) };
  return { repository, consumption: new HimGoalMotivationConsumptionService(repository as never) };
};

const NONE = { contractVersion: 1, guidanceState: 'NONE', directive: 'DEFAULT' };
const REDUCE = { contractVersion: 1, guidanceState: 'ACTIVE', directive: 'REDUCE_GOAL_ACTION_BURDEN' };

describe('HimGoalMotivationConsumptionService (QHIA-010)', () => {
  describe('one authoritative composed read', () => {
    it('performs EXACTLY ONE repository read with the exact authenticated user and owned session', async () => {
      ...
      expect(repository.readSessionGoalMotivation).toHaveBeenCalledTimes(1);
      expect(repository.readSessionGoalMotivation).toHaveBeenCalledWith('token', USER, SESSION);
    });
    ...
    it('rejects a malformed identity before any external request is issued', async () => {
      for (const [userId, sessionId] of [
        ...
        const { repository, consumption } = service([unboundRow()]);
        await expect(consumption.read(userId, 'token', sessionId)).rejects.toThrow('INVALID_GOAL_MOTIVATION_REQUEST');
        expect(repository.readSessionGoalMotivation).not.toHaveBeenCalled();
      }
    });
  });
  ...
  describe('frozen semantic mapping', () => {
    ...
    it('exposes exactly one non-default directive: there is no second, stronger, or opposite direction', () => {
      const types = readFileSync(`${__dirname}/him-goal-motivation-consumption.types.ts`, 'utf8');
      const directiveLine = types.split('\n').find((line) => line.startsWith('export type HimGoalMotivationDirective'));
      expect(directiveLine).toBe("export type HimGoalMotivationDirective = 'DEFAULT' | 'REDUCE_GOAL_ACTION_BURDEN';");
    });
    ...
    it('treats an UNASSESSED latest value as no effect, never as a low value', async () => {
      const row = { ...boundRow(2), value_state: 'UNASSESSED', numeric_value: null, canonical_binding_id: null } as unknown as HimGoalMotivationSourceRow;
    ...
    it('treats an INVALIDATED latest value as no effect', async () => {
      const row = { ...boundRow(1), validity_status: 'INVALIDATED' } as unknown as HimGoalMotivationSourceRow;
      await expect(service([row]).consumption.read(USER, 'token', SESSION)).resolves.toEqual(NONE);
    });

    it('treats an incompatible ACTIVE measurement binding as no effect - the QHIA-004 rule stays delegated', async () => {
      const row = { ...boundRow(1), active_binding_id: OTHER_BINDING } as unknown as HimGoalMotivationSourceRow;
      await expect(service([row]).consumption.read(USER, 'token', SESSION)).resolves.toEqual(NONE);
    });

    it('returns the deterministic no-effect result when no ACTIVE Goal is bound', async () => {
    ...
    it('performs no arithmetic, severity stacking, trend, baseline, decay, or confidence inference', async () => {
      // Every KNOWN value produces exactly one of the two frozen shapes, and
      // repeated reads of different values never accumulate: there is no state.
      ...
      expect(first).toEqual(REDUCE);
      const source = readFileSync(`${__dirname}/him-goal-motivation-consumption.service.ts`, 'utf8');
    ...
    it('emits only the two frozen guidance shapes and never leaks a value, id, target text, or provenance', async () => {
      for (const rows of [[unboundRow()], [boundRow(1)], [boundRow(4)], [boundRow(null)]]) {
        ...
        for (const forbidden of [GOAL, SESSION, USER, BINDING, 'hse.motivation', 'GOAL_BOUND', 'VERY_LOW', '2026-08-28']) {
  ...
  describe('exact frozen semantic identity (adversarial)', () => {
    ...
    // HBS/HRS metrics are legitimately UNRESOLVED / null. A row can therefore be
    // INTERNALLY COHERENT - definition and source semantic metadata agree, the
    // ACTIVE canonical binding is valid, the value is a KNOWN 1 or 2 - and still
    // not be an hse.motivation@1 reading at all. Canonical Motivation is
    ...
    // is precisely the drift that must never become action-pacing guidance.
    const semanticallyDrifted = (semanticType: string, numericValue: number): HimGoalMotivationSourceRow => ({
    ...
    it.each([
      ...
      'rejects a coherent RESOLVED / %s row carrying a KNOWN %i and never yields REDUCE_GOAL_ACTION_BURDEN',
    ...
    it('never produces guidance for ANY resolved semantic reading other than STATE, at any scale value', async () => {
    ...
    it('rejects an unresolved semantic mapping under the Motivation metric key', async () => {
      const row = {
        ...boundRow(1),
    ...
    it('rejects a drifted semantic reading on the UNKNOWN routes too, never mapping it to a benign NONE', async () => {
      ...
      for (const row of unknownRoutes) {
        const { consumption } = service([row as unknown as HimGoalMotivationSourceRow]);
        await expect(consumption.read(USER, 'token', SESSION)).rejects.toThrow('INTEGRITY_FAILURE');
      }
    });
    ...
    it('leaves the SHARED QHIA-004 projection generic: it still accepts the semantic readings it is designed for', () => {
      // Anti-over-fix control. The guard above must live at the QHIA-010
      ...
      // UNRESOLVED / null HBS metric - would start failing, and every other HIM
      // consumer with them.
      const alignmentRow = {
        ...boundRow(4),
        ...
        valid_context_kinds: ['GOAL'],
      } as unknown as HimGoalMotivationSourceRow;
      expect(projectHimContextualCurrentSlot(alignmentRow, 'hgs.purpose-alignment', 1, 'GOAL', GOAL))
        .toMatchObject({
      ...
      const unresolvedRow = {
        ...boundRow(2),
        metric_key: 'hbs.consistency', hif_owner: 'HBS',
        semantic_mapping_status: 'UNRESOLVED', semantic_type: null,
        source_metric_key: 'hbs.consistency',
        source_semantic_mapping_status: 'UNRESOLVED', source_semantic_type: null,
        valid_context_kinds: ['GOAL'],
      } as unknown as HimGoalMotivationSourceRow;
      expect(projectHimContextualCurrentSlot(unresolvedRow, 'hbs.consistency', 1, 'GOAL', GOAL))
        .toMatchObject({
          metricKey: 'hbs.consistency', hifOwner: 'HBS',
          semanticMappingStatus: 'UNRESOLVED', semanticType: null,
          knowledgeState: 'KNOWN', numericValue: 2,
        });

      // And the same generic projection accepts the drifted hse.motivation row
      // the QHIA-010 boundary rejects - proving the rejection is the CONSUMER's
      // bound, not a change to the shared contract.
      expect(projectHimContextualCurrentSlot(semanticallyDrifted('READINESS', 1), 'hse.motivation', 1, 'GOAL', GOAL))
        .toMatchObject({ knowledgeState: 'KNOWN', numericValue: 1, semanticType: 'READINESS' });
    });
  });
  ...
  describe('fail-closed integrity', () => {
    ...
    it('rejects an unknown or malformed binding_state rather than defaulting it to bound or unbound', async () => {
      for (const bindingState of ['BOUND', 'ACTIVE', 'INFERRED_GOAL', 'ACTIVE_SITUATION_BOUND', 'ACTIVE_DECISION_BOUND', 'NO_ACTIVE_SITUATION', '', null, undefined]) {
    ...
    it('rejects an UNBOUND answer that still carries any metric or binding fragment', async () => {
      for (const fragment of [
        { binding_context_id: GOAL }, { metric_key: 'hse.motivation' }, { numeric_value: 1 },
        { has_canonical_current_value: false }, { context_kind: 'GOAL' }, { context_id: GOAL },
    ...
    it('rejects a BOUND answer whose delegated row answers for another metric, version, owner, kind, or context', async () => {
      for (const drift of [
        { metric_key: 'hse.energy' }, { metric_key: 'hse.stress' }, { metric_key: 'hgs.purpose-alignment' },
        { definition_version: 2 }, { hif_owner: 'HBS' }, { hif_owner: 'HGS' },
        // The dormant SITUATION context of this very metric is the most
        // dangerous drift of all: hse.motivation@1 is measurement-valid there,
        // so the row would otherwise look structurally perfect.
        { context_kind: 'SITUATION', source_context_kind: 'SITUATION' },
        { context_kind: 'CONVERSATION_SESSION' }, { context_kind: 'DECISION' },
        { context_id: OTHER_GOAL }, { slot_order: 2 },
        { valid_context_kinds: ['SITUATION'] }, { calculation_status: 'UNCALIBRATED' },
        { semantic_mapping_status: 'UNRESOLVED' }, { semantic_type: null },
      ]) await failsClosed([{ ...boundRow(1), ...drift }]);
    });

    it('rejects a BOUND answer with a missing or malformed authoritative Goal identity', async () => {
    ...
    it('rejects a BOUND answer whose delegated canonical row answers for a different Goal than the bound one', async () => {
      await failsClosed([{ ...boundRow(1), source_context_id: OTHER_GOAL }]);
      await failsClosed([{ ...boundRow(1), source_context_kind: 'SITUATION' }]);
      await failsClosed([{ ...boundRow(1), binding_context_id: OTHER_GOAL, context_id: OTHER_GOAL }]);
    });
    ...
    it('propagates a repository failure so the caller degrades it and telemetry stays honest', async () => {
      const repository = { readSessionGoalMotivation: jest.fn().mockRejectedValue(new Error('private transport failure')) };
      const consumption = new HimGoalMotivationConsumptionService(repository as never);
      await expect(consumption.read(USER, 'token', SESSION)).rejects.toThrow('private transport failure');
    });
  });

  describe('scope discipline', () => {
    it('activates exactly GOAL + hse.motivation@1 and reaches no other context, metric, or authority', () => {
      const source = readFileSync(`${__dirname}/him-goal-motivation-consumption.service.ts`, 'utf8');
      const types = readFileSync(`${__dirname}/him-goal-motivation-consumption.types.ts`, 'utf8');
      const executable = [source, types].join('\n').split('\n').filter((line) => !line.trim().startsWith('//') && !line.trim().startsWith('*') && !line.trim().startsWith('/*')).join('\n');
      expect(executable).toContain("'GOAL'");
      expect(executable).toContain("'hse.motivation'");
      for (const forbidden of [
        "'SITUATION'", "'DECISION'", "'RELATIONSHIP'", "'GLOBAL'", "'CONVERSATION_SESSION'",
        'hse.energy', 'hse.stress', 'hse.attention', 'hse.self-confidence',
        'hbs.', 'hrs.', 'hgs.',
    ...
    it('keeps Situation-bound Motivation dormant across the whole QHIA-010 boundary', () => {
      // The other valid measurement context of this same metric appears nowhere
      // executable in this task's own application surface - not as a constant,
      // a request, a fallback, or an inference from the Goal reading.
      for (const file of [
        'him-goal-motivation-consumption.service.ts',
        'him-goal-motivation-consumption.types.ts',
        'him-goal-motivation.repository.ts',
      ]) {
        const executable = readFileSync(`${__dirname}/${file}`, 'utf8')
          .split('\n').filter((line) => !line.trim().startsWith('//') && !line.trim().startsWith('*') && !line.trim().startsWith('/*')).join('\n');
        expect(executable).not.toContain('SITUATION');
        expect(executable).not.toContain('situationBound');
      }
    });
    ...
    it('reuses the shared frozen QHIA-004 projection instead of duplicating current-intelligence validation', () => {
      ...
      // no Goal-specific rule, no Motivation metric identity, and no
      ...
      const projectionExecutable = readFileSync(`${__dirname}/him-contextual-current-projection.ts`, 'utf8')
        .split('\n').filter((line) => !line.trim().startsWith('//') && !line.trim().startsWith('*') && !line.trim().startsWith('/*')).join('\n');
      for (const forbidden of ['hse.motivation', 'GoalMotivation', 'REDUCE_GOAL_ACTION_BURDEN', "'STATE'", "'GOAL'"]) {
        expect(projectionExecutable).not.toContain(forbidden);
      }
    });

    it('leaves the QHIA-007 and QHIA-008 boundaries byte-identical', () => {
      // No shared abstraction and no edit to the two proven consumers: QHIA-010
      // adds a third narrow boundary beside them.
      ...
      expect(situationStressService).not.toContain('GoalMotivation');
      expect(situationStressService).not.toContain('hse.motivation');
      const decisionAttentionService = readFileSync(`${__dirname}/him-decision-attention-consumption.service.ts`, 'utf8');
      expect(decisionAttentionService).toContain('HIM_DECISION_ATTENTION_METRIC_KEY');
      expect(decisionAttentionService).not.toContain('GoalMotivation');
      expect(decisionAttentionService).not.toContain('hse.motivation');
      const goalRepository = readFileSync(`${__dirname}/him-goal-motivation.repository.ts`, 'utf8');
      expect(goalRepository).toContain("'rpc/read_him_session_goal_motivation_v1'");
    });
  });
});
