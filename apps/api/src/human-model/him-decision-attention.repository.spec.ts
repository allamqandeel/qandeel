...
// QHIA-008 transport contract. The single architectural requirement of this
// boundary is that ONE QHIA-008 foreground read costs EXACTLY ONE external
// Data API / PostgREST request against the one narrow migration-0057
...
describe('HimDecisionAttentionRepository', () => {
  it('reads Decision-bound attention through EXACTLY ONE Data API request with the exact RPC path and body', async () => {
    const rows = [{ binding_state: 'NO_ACTIVE_DECISION' }];
    const dataApi = { request: jest.fn().mockResolvedValue(rows) };
    const repository = new HimDecisionAttentionRepository(dataApi as never);
    const returned = await repository.readSessionDecisionAttention('token', USER, SESSION);
    expect(dataApi.request).toHaveBeenCalledTimes(1);
    expect(dataApi.request).toHaveBeenCalledWith('token', 'rpc/read_him_session_decision_attention_v1', {
      method: 'POST',
  ...
  it('sends no context kind, context id, metric key, metric list, or target: the callable surface cannot be aimed', async () => {
    ...
    const repository = new HimDecisionAttentionRepository(dataApi as never);
    await repository.readSessionDecisionAttention('token', USER, SESSION);
    ...
    expect(JSON.stringify(body)).not.toContain('hse.attention');
  });
  ...
  it('normalizes a missing payload to an empty array without issuing a second request', async () => {
    ...
    const repository = new HimDecisionAttentionRepository(dataApi as never);
    await expect(repository.readSessionDecisionAttention('token', USER, SESSION)).resolves.toEqual([]);
    expect(dataApi.request).toHaveBeenCalledTimes(1);
  });
  ...
  it('propagates a transport failure after exactly one attempt: no retry and no second round trip', async () => {
    ...
    const repository = new HimDecisionAttentionRepository(dataApi as never);
    await expect(repository.readSessionDecisionAttention('token', USER, SESSION)).rejects.toThrow('transport failure');
    expect(dataApi.request).toHaveBeenCalledTimes(1);
  });
  ...
  it('routes only through the one narrow RPC path - no direct table route and no query surface exists', async () => {
    ...
    expect(path).toBe('rpc/read_him_session_decision_attention_v1');
  ...
  it('depends on exactly one Data API collaborator and exposes exactly the one read method', () => {
    ...
    expect(HimDecisionAttentionRepository.length).toBe(1);
    const methods = Object.getOwnPropertyNames(HimDecisionAttentionRepository.prototype).filter((name) => name !== 'constructor');
    expect(methods).toEqual(['readSessionDecisionAttention']);
  });

  it('contains exactly one Data API call site and references no other repository or RPC in its source', () => {
    const source = readFileSync(`${__dirname}/him-decision-attention.repository.ts`, 'utf8');
    ...
    expect(source).toContain("'rpc/read_him_session_decision_attention_v1'");
