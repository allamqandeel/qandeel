...
// QHIA-010 transport contract. The single architectural requirement of this
// boundary is that ONE QHIA-010 direct foreground read costs EXACTLY ONE
// external Data API / PostgREST request against the one narrow migration-0059
...
// Conversation Orchestrator never calls it: the turn reaches Goal Motivation
// only through the single aggregate-v2 transport, which wraps this authority
// server-side. That separation is proven in the Orchestrator race tests and in
// the Full Intelligence smoke transport census.
const USER = '00000000-0000-4000-8000-000000000001';
const SESSION = '00000000-0000-4000-8000-000000000002';

describe('HimGoalMotivationRepository', () => {
  it('reads Goal-bound motivation through EXACTLY ONE Data API request with the exact RPC path and body', async () => {
    const rows = [{ binding_state: 'NO_ACTIVE_GOAL' }];
    const dataApi = { request: jest.fn().mockResolvedValue(rows) };
    const repository = new HimGoalMotivationRepository(dataApi as never);
    const returned = await repository.readSessionGoalMotivation('token', USER, SESSION);
    expect(dataApi.request).toHaveBeenCalledTimes(1);
    expect(dataApi.request).toHaveBeenCalledWith('token', 'rpc/read_him_session_goal_motivation_v1', {
      method: 'POST',
  ...
  it('sends no context kind, context id, metric key, metric list, or target: the callable surface cannot be aimed', async () => {
    ...
    const repository = new HimGoalMotivationRepository(dataApi as never);
    await repository.readSessionGoalMotivation('token', USER, SESSION);
    ...
    for (const forbidden of ['GOAL', 'SITUATION', 'hse.motivation']) expect(serialized).not.toContain(forbidden);
  });
  ...
  it('normalizes a missing payload to an empty array without issuing a second request', async () => {
    ...
    const repository = new HimGoalMotivationRepository(dataApi as never);
    await expect(repository.readSessionGoalMotivation('token', USER, SESSION)).resolves.toEqual([]);
    expect(dataApi.request).toHaveBeenCalledTimes(1);
  });
  ...
  it('propagates a transport failure after exactly one attempt: no retry and no second round trip', async () => {
    ...
    const repository = new HimGoalMotivationRepository(dataApi as never);
    await expect(repository.readSessionGoalMotivation('token', USER, SESSION)).rejects.toThrow('transport failure');
    expect(dataApi.request).toHaveBeenCalledTimes(1);
  });
  ...
  it('routes only through the one narrow RPC path - no direct table route and no query surface exists', async () => {
    ...
    expect(path).toBe('rpc/read_him_session_goal_motivation_v1');
  ...
  it('depends on exactly one Data API collaborator and exposes exactly the one read method', () => {
    ...
    expect(HimGoalMotivationRepository.length).toBe(1);
    const methods = Object.getOwnPropertyNames(HimGoalMotivationRepository.prototype).filter((name) => name !== 'constructor');
    expect(methods).toEqual(['readSessionGoalMotivation']);
  });

  it('contains exactly one Data API call site and references no other repository or RPC in its source', () => {
    const source = readFileSync(`${__dirname}/him-goal-motivation.repository.ts`, 'utf8');
    ...
    expect(source).toContain("'rpc/read_him_session_goal_motivation_v1'");
