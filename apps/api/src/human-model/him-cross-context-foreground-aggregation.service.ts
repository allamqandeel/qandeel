...
import {
  HIM_CROSS_CONTEXT_FOREGROUND_V2_SLOTS,
...
// by QHIA-010 to the fixed three-slot v2 contract.
//
// One repository call - and therefore exactly ONE external Data API request -
// carries ALL THREE already-approved foreground channels for the turn. This
...
// channel; it never repairs, defaults, or substitutes malformed child data; and
// it holds no threshold, wording, relevance rule, or measurement authority.
// Stress meaning stays inside QHIA-007, Attention meaning inside QHIA-008, and
// Motivation meaning inside QHIA-010 - there is deliberately no shared switch,
// no generic metric-to-behaviour mapper, and no place for a slot to acquire
// meaning by accident.
//
// Fail-closed, atomically. A malformed envelope - not an array, not exactly
// three rows, a duplicate/missing/unknown slot, or a slot out of its frozen
...
// an OPTIONAL foreground enrichment whose caller degrades by omitting ALL THREE
// guidance fields, never by retrying, never by falling back to the aggregate-v1
// endpoint or to the direct QHIA-007/QHIA-008/QHIA-010 requests, and never by
// inventing a NONE answer the authorities did not give.
@Injectable()
export class HimCrossContextForegroundAggregationService {
  ...
  async read(userId: string, token: string, sessionId: string): Promise<HimCrossContextForegroundGuidance> {
    ...
    const [situationRow, decisionRow, goalRow] = this.partition(rows);
  ...
  // order, because migration 0059 wraps the frozen two-slot v1 aggregate and
  ...
  private partition(rows: HimCrossContextForegroundEnvelopeRow[]): HimCrossContextForegroundEnvelopeRow[] {
    if (!Array.isArray(rows) || rows.length !== HIM_CROSS_CONTEXT_FOREGROUND_V2_SLOTS.length)
      throw new Error('INTEGRITY_FAILURE');
    return HIM_CROSS_CONTEXT_FOREGROUND_V2_SLOTS.map(({ order, slot }, index) => {
