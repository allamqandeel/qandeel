...
import { calculateHbsConsistency,HBS_CONSISTENCY_MODEL,HBS_CONSISTENCY_WINDOW_MS,type HbsConsistencyObservation } from './hbs-consistency.model';
...
const observation=(overrides:Partial<HbsConsistencyObservation>={}):HbsConsistencyObservation=>({
 observationId:'obs-1',measurementEventId:'event-1',userId:'user-1',metricKey:'hbs.consistency',definitionVersion:1,
 ...
 instrumentId:'hbs.consistency.direct-target-bound-seven-day-report',instrumentVersion:1,scaleContractReference:'hbs.consistency.frequency-5.v1',scaleVersion:1,
...
const input=(o?:HbsConsistencyObservation,overrides:Partial<HimMetricCalculationInput>={}):HimMetricCalculationInput=>({
 metricKey:'hbs.consistency',definitionVersion:1,modelId:HBS_CONSISTENCY_MODEL.modelId,modelVersion:1,
...
describe('HBS Consistency measurement model v1',()=>{
 it('freezes the exact calibrated target-bound seven-day model identity',()=>{
  expect(HBS_CONSISTENCY_MODEL).toMatchObject({
   modelId:'hbs.consistency.direct-structured-seven-day-self-report',modelVersion:1,targetMetricKey:'hbs.consistency',targetDefinitionVersion:1,
   lifecycle:'CALIBRATED',environment:'PRODUCTION',canonicalOwner:'QANDEEL_HIM_GOVERNANCE',
   methodType:'DIRECT_STRUCTURED_TARGET_BOUND_PERIOD_REPORT',scaleContractReference:'hbs.consistency.frequency-5.v1',
  ...
  expect(HBS_CONSISTENCY_WINDOW_MS).toBe(604_800_000);
 });
 it.each([['NEVER',1],['RARELY',2],['SOMETIMES',3],['OFTEN',4],['ALMOST_ALWAYS',5]] as const)('scores %s as ordinal %i with no interval arithmetic semantics',(code,value)=>{
  const result=calculateHbsConsistency(input(observation({responseCode:code})));
 ...
 it.each(['INSUFFICIENT_REPEATED_OPPORTUNITIES','NOT_SURE'] as const)('treats %s as UNASSESSED null, never zero',code=>{
  const result=calculateHbsConsistency(input(observation({responseCode:code})));
 ...
 it('treats a missing observation as UNASSESSED null',()=>{
  expect(calculateHbsConsistency(input(undefined))).toMatchObject({resultState:'UNASSESSED',numericValue:null,missingInputKeys:['observation']});
 });
 it('preserves contradiction as PRESENT_UNRESOLVED null without assessing',()=>{
  expect(calculateHbsConsistency(input(observation(),{contradictoryEvidenceRefs:['memory:20000000-0000-4000-8000-000000000002']})))
 ...
 it.each([
  ...
  expect(()=>calculateHbsConsistency(input(invalid))).toThrow(BadRequestException);
 });
 // Sibling-metric and Avoidance observations can never be scored under this
 // model: an Initiative or Avoidance report carries its own metric key,
 // instrument, scale, and vocabulary, and every one of those mismatches is
 // rejected. Consistency is never derived from a sibling.
 it.each([
  ['a sibling Initiative observation',observation({metricKey:'hbs.initiative' as never,instrumentId:'hbs.initiative.direct-target-bound-seven-day-report' as never,scaleContractReference:'hbs.initiative.frequency-5.v1' as never})],
  ...
  ['a sibling special code on its own vocabulary',observation({responseCode:'NO_CLEAR_SELF_OWNED_OPPORTUNITY' as never})],
 ...
 it('depends only on its own observation: extra sibling inputs change nothing (no inverse or composite)',()=>{
  const alone=calculateHbsConsistency(input(observation({responseCode:'ALMOST_ALWAYS'})));
  const crowded=calculateHbsConsistency({...input(observation({responseCode:'ALMOST_ALWAYS'})),inputs:{observation:observation({responseCode:'ALMOST_ALWAYS'}),avoidanceObservation:{responseCode:'ALMOST_ALWAYS'},initiativeObservation:{responseCode:'NEVER'}}});
 ...
 it.each([
  ...
  expect(()=>calculateHbsConsistency(input(invalid))).toThrow('Consistency exact target/context mismatch.');
 });
 it.each([
  ...
  expect(()=>calculateHbsConsistency(input(invalid))).toThrow('Consistency seven-day observation window mismatch.');
 });
 it('accepts the original preserved window on a later corrected report timestamp',()=>{
  ...
  expect(calculateHbsConsistency(input(corrected)).resultState).toBe('ASSESSED');
 });
 it('rejects a superseded observation',()=>{
  expect(()=>calculateHbsConsistency(input(observation({superseded:true})))).toThrow('Superseded Consistency observation cannot be calculated.');
 });
});
