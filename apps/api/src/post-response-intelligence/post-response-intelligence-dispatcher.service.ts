...
export class PostResponseIntelligenceDispatcherService{
 ...
 async dispatch(raw:string):Promise<boolean>{let event:RuntimeEventEnvelope;try{event=JSON.parse(raw)as RuntimeEventEnvelope;}catch{return true;}if(!isValidRuntimeEventEnvelope(event))return true;if(event.event_type!=='ConversationTurnCompleted')return true;const v2=event.event_version==='2.0';const execution=await this.ledger.acquire({id:randomUUID(),eventId:event.event_id,userId:event.subject_user_id,sessionId:event.subject_session_id,sourceTurnId:event.subject_turn_id,eventVersion:event.event_version,processingPath:(event.payload.processing_path??null)as'FAST'|'DEEP'|null,safetyDisposition:v2?event.payload.safety_disposition as'ALLOW'|'GUIDED'|'BLOCK':null});if(execution.state!=='RUNNING')return true;const effects=await this.ledger.effects(execution.id);if(effects.some(effect=>effect.state==='CLAIMED'))return this.terminal(execution,'QUARANTINED','INDETERMINATE_EFFECT','EFFECT_RECOVERY');if(execution.attempt_count>=5)return this.terminal(execution,'QUARANTINED','MAX_ATTEMPTS','DELIVERY');if(!v2)return this.terminal(execution,'SKIPPED','LEGACY_UNSUPPORTED','VALIDATION');const authorized=await this.authority.authorize(event);if(authorized.outcome!=='AUTHORIZED'||!authorized.context)return this.terminal(execution,'QUARANTINED',authorized.outcome==='NOT_AUTHORIZED_OWNER_MISMATCH'?'CANONICAL_MISMATCH':'AUTHORITY_REJECTED','AUTHORITY');if(execution.safety_disposition!=='ALLOW')return this.terminal(execution,'SKIPPED','SAFETY_SKIPPED','SAFETY');const context=authorized.context;const turn=await this.enrichment.readCanonicalSourceTurn(context);if(turn.processing_path!==execution.processing_path)return this.terminal(execution,'QUARANTINED','CANONICAL_MISMATCH','CANONICAL_REREAD');
  ...
  // QIR-005: the HARD lifecycle provider budget is opened HERE, from the durable
  // effect ledger and nothing else.
  //
  // Exactly three post-response effects are provider-backed - ASSOCIATION_PROVIDER,
  // INTENT_PROVIDER and CANDIDATE_PROVIDER - so one durable execution may spend at
  // most three provider slots, one per effect. Because the spent set is
  // reconstructed from durable state alone, the budget is a property of the
  // EXECUTION, not of this delivery: a duplicate Redis delivery, a reclaimed
  // redelivery, a redispatch, a retry or a process restart re-derives the same
  // spent set instead of resetting it, and there is no per-delivery allowance.
  //
  // Both CLAIMED and COMPLETED provider effects count as spent, and a spent slot
  // is never refunded: after a durable claim the runtime can never prove that no
  // external provider request was emitted. A pre-existing CLAIMED effect is not
  // replayed here either - it already quarantined above under the canonical
  // indeterminate-effect rule.
  const budget=this.providerBudget.open(effects,execution.processing_path);
  ...
  if(completed.has('HYPOTHESIS_PERSISTENCE'))return this.resumeGenerationConfidence(execution,effects,budget);
  ...
  if(freshEvidenceId){if(completed.has('ASSOCIATION_PROVIDER')){const recovered=recoverAssociationResult(effects.find(effect=>effect.effect_key==='ASSOCIATION_PROVIDER')!,freshEvidenceId);if(recovered.status==='INDETERMINATE')return this.terminal(execution,'QUARANTINED','INDETERMINATE_EFFECT','ASSOCIATION_RECOVERY');budget.recover('ASSOCIATION_PROVIDER');association=recovered;}else{const preparation=await this.association.prepare(context,freshEvidenceId);if(preparation.status==='NOT_AUTHORIZED')return this.terminal(execution,'QUARANTINED','AUTHORITY_REJECTED','ASSOCIATION_PREPARATION');if(preparation.status==='PREPARED'){const durable=await this.completeAssociation(execution,budget,completed,event,context,preparation.snapshot);if(durable==='PROVIDER_BUDGET_EXHAUSTED')return this.providerBudgetExhausted(execution);if(!durable)return true;association=durable.code==='NO_ASSOCIATION'?{status:'NO_ASSOCIATION'}:{status:'AUTHORIZED_COMMANDS',commands:durable.commands};}}}
  ...
  if(persistedIntent){const recovered=recoverDurableIntentProviderResult(persistedIntent,execution);if(recovered.status==='INDETERMINATE')return this.terminal(execution,'QUARANTINED','INDETERMINATE_EFFECT','INTENT_RECOVERY');budget.recover('INTENT_PROVIDER');if(recovered.status==='NOT_AUTHORIZED')return this.terminal(execution,'SKIPPED','INTENT_NOT_AUTHORIZED','INTENT');intent=recovered.intent;}
  else{
   // QIR-005: fresh Intent work passes the centralized provider-budget gate
   // BEFORE its durable claim. An exhausted budget issues ZERO provider
   // transport and claims nothing; a failed durable claim consumes no slot and
   // also calls no provider; only a SUCCEEDED claim spends the slot, and only
   // then is the single provider transport attempt made.
   if(budget.authorize('INTENT_PROVIDER')==='EXHAUSTED')return this.providerBudgetExhausted(execution);
   if(!await this.ledger.claim(execution.id,'INTENT_PROVIDER'))return true;budget.spend('INTENT_PROVIDER');let extracted:Awaited<ReturnType<HypothesisGenerationIntentExtractionService['extract']>>;try{extracted=await this.extraction.extract({currentTurn:{id:turn.id,sessionId:turn.session_id,role:'USER',status:'COMPLETED',text:turn.content},eligibility:assessment.eligibility,triggerReason:assessment.triggerClassification.reason,eligibleEvidence:assessment.eligibleEvidence});}catch{await this.ledger.finish(execution.id,'QUARANTINED','INDETERMINATE_EFFECT','INTENT_PROVIDER');return true;}
  ...
  if(persistedCandidate){const recovered=recoverCandidateProviderResult(persistedCandidate,intent);if(recovered.status==='INDETERMINATE')return this.terminal(execution,'QUARANTINED','INDETERMINATE_EFFECT','CANDIDATE_RECOVERY');budget.recover('CANDIDATE_PROVIDER');candidate=recovered;}
   ...
   // T-03C R2: the authorized subject-grounding universe is built by the SERVER
   // from the committed B1 / B2 / B3 truth of the execution's Session and
   // durably recorded BEFORE the Candidate claim - a failure here claims
   // nothing and calls no provider, exactly like the HIM read - and the
   // provider only ever sees its opaque per-execution handles.
   //
   // T-03C R3: that truth is cut at the IMMUTABLE causal semantic frontier of
   // this execution's own source finalized exchange, so the schedule the
   // worker happens to run on cannot change it. A worker that arrives before
   // the source exchange finished its FINAL semantic establishment gets the
   // explicit SOURCE_SEMANTIC_FRONTIER_NOT_ESTABLISHED condition instead of a
   // universe: it is NOT an empty universe and NOT a database failure. It
   // stores nothing, spends no provider budget slot, claims nothing, calls no
   // provider and returns non-terminal (non-ACK), so the existing bounded
   // delivery-attempt policy retries it once the FINAL semantic phase has
   // committed - exactly the recovery path a HIM read failure already uses.
   let subjectGroundingUniverse:AuthorizedSubjectGroundingUniverse;
   try{const resolved=await this.ledger.buildSubjectGroundingUniverse(execution.id);
    if(resolved.status===SOURCE_SEMANTIC_FRONTIER_NOT_ESTABLISHED)return false;
    subjectGroundingUniverse=resolved.universe;}catch{return false;}
   // QIR-005: the centralized provider-budget gate sits between the HIM read and
   // the Candidate claim, so BOTH fail-closed rules hold at once - a HIM
   // read/integrity failure still claims nothing and calls no provider, and an
   // exhausted budget still issues ZERO provider transport and leaves
   // CANDIDATE_PROVIDER unclaimed.
   if(budget.authorize('CANDIDATE_PROVIDER')==='EXHAUSTED')return this.providerBudgetExhausted(execution);
   if(!await this.ledger.claim(execution.id,'CANDIDATE_PROVIDER'))return true;budget.spend('CANDIDATE_PROVIDER');let plan:DurableCandidateProviderResult;try{plan=await this.enrichment.generateHypothesisCandidatePlan(context,assembled.request,this.generator,himContext,subjectGroundingUniverse);}catch{return this.terminal(execution,'QUARANTINED','INDETERMINATE_EFFECT','CANDIDATE_PROVIDER');}
 ...
 private async resumeGenerationConfidence(execution:IntelligenceExecution,effects:readonly IntelligenceEffectState[],budget:PostResponseProviderBudget):Promise<boolean>{
 ...
 private async completeAssociation(execution:IntelligenceExecution,budget:PostResponseProviderBudget,completed:Set<IntelligenceEffect>,event:RuntimeEventEnvelope,context:BackgroundIntelligenceExecutionContext,snapshot:HypothesisEvidenceAssociationSnapshot):Promise<DurableAssociationResult|'PROVIDER_BUDGET_EXHAUSTED'|undefined>{if(budget.authorize('ASSOCIATION_PROVIDER')==='EXHAUSTED')return'PROVIDER_BUDGET_EXHAUSTED';if(!await this.ledger.claim(execution.id,'ASSOCIATION_PROVIDER'))return undefined;budget.spend('ASSOCIATION_PROVIDER');let durable;try{durable=toDurableAssociationResult(await this.association.proposeAndAuthorize(event,context,snapshot));}catch{await this.ledger.finish(execution.id,'QUARANTINED','INDETERMINATE_EFFECT','ASSOCIATION_PROVIDER');return undefined;}if(!durable){await this.ledger.finish(execution.id,'QUARANTINED','INDETERMINATE_EFFECT','ASSOCIATION_PROVIDER');return undefined;}if(!await this.ledger.completeAssociation(execution.id,durable))return undefined;completed.add('ASSOCIATION_PROVIDER');return durable;}
 // QIR-005 hard provider-budget exhaustion. Under the valid v1 DAG this is
 // unreachable during a legitimate execution, so it is an integrity/contract
 // violation rather than ordinary optional degradation: ZERO provider transport
 // has happened when this runs, no effect was claimed for the refused work, no
 // intelligence result is fabricated, no stale value is substituted, the cap is
 // never raised dynamically, and the execution fails closed as a TERMINAL
 // quarantine at the dedicated PROVIDER_BUDGET stage.
 private providerBudgetExhausted(execution:IntelligenceExecution):Promise<true>{
  return this.terminal(execution,'QUARANTINED',POST_RESPONSE_PROVIDER_BUDGET_EXHAUSTED_OUTCOME,POST_RESPONSE_PROVIDER_BUDGET_EXHAUSTED_STAGE);}
