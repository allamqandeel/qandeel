...
describe('PostResponseIntelligenceDispatcherService',()=>{
 ...
 describe('Finding 08 - durable sequential generation',()=>{
  ...
  // T-03C R2: the server-built subject-grounding universe is read BEFORE the
 // Candidate claim and handed to the one provider stage; a universe failure
 // claims nothing, calls no provider and returns non-terminal.
 it('builds the subject-grounding universe before the Candidate claim, passes it to the provider stage, and claims nothing when it fails',async()=>{
  const s=eligibleRun(setup());s.extraction.extract.mockResolvedValue({status:'AUTHORIZED',intent:intent()} as never);
  const universe={executionId:id.execution,frontierSp:5,entries:[{handle:'22d3c5d1-02cc-55e5-97c8-7b5563e5332f',subjectText:'أحمد',startedSp:3,lastAttentionSp:5}]};
  s.ledger.buildSubjectGroundingUniverse.mockResolvedValue({status:'ESTABLISHED',universe} as never);
  await s.service.dispatch(JSON.stringify(event()));
  expect(s.ledger.buildSubjectGroundingUniverse).toHaveBeenCalledWith(id.execution);
  const candidateClaim=(s.ledger.claim as jest.Mock).mock.calls.findIndex(call=>call[1]==='CANDIDATE_PROVIDER');
  expect(candidateClaim).toBeGreaterThanOrEqual(0);
  expect(s.ledger.buildSubjectGroundingUniverse.mock.invocationCallOrder[0]).toBeLessThan((s.ledger.claim as jest.Mock).mock.invocationCallOrder[candidateClaim]);
  expect(s.enrichment.generateHypothesisCandidatePlan.mock.calls[0][4]).toEqual(universe);
  const failing=eligibleRun(setup());failing.extraction.extract.mockResolvedValue({status:'AUTHORIZED',intent:intent()} as never);
  failing.ledger.buildSubjectGroundingUniverse.mockRejectedValue(new Error('POST_RESPONSE_DATABASE_UNAVAILABLE'));
  await expect(failing.service.dispatch(JSON.stringify(event()))).resolves.toBe(false);
  expect((failing.ledger.claim as jest.Mock).mock.calls.some(call=>call[1]==='CANDIDATE_PROVIDER')).toBe(false);
  expect(failing.enrichment.generateHypothesisCandidatePlan).not.toHaveBeenCalled();
  expect(failing.ledger.finish).not.toHaveBeenCalled();
 });
 // T-03C R3 (CF-01): a worker that arrives before its source finalized
 // exchange finished FINAL semantic establishment has no causal frontier. The
 // stable technical condition is NOT collapsed into an empty universe and NOT
 // into a database failure: nothing is claimed, no provider budget slot is
 // spent, no provider is called, no execution is finished, and the dispatch
 // returns non-terminal so the existing bounded delivery policy retries.
 it('spends no provider budget and calls no provider while the source causal semantic frontier is not established',async()=>{
  const s=eligibleRun(setup());s.extraction.extract.mockResolvedValue({status:'AUTHORIZED',intent:intent()} as never);
  s.ledger.buildSubjectGroundingUniverse.mockResolvedValue({status:'SOURCE_SEMANTIC_FRONTIER_NOT_ESTABLISHED'} as never);
  await expect(s.service.dispatch(JSON.stringify(event()))).resolves.toBe(false);
  expect(s.ledger.buildSubjectGroundingUniverse).toHaveBeenCalledWith(id.execution);
  expect((s.ledger.claim as jest.Mock).mock.calls.some(call=>call[1]==='CANDIDATE_PROVIDER')).toBe(false);
  expect(s.enrichment.generateHypothesisCandidatePlan).not.toHaveBeenCalled();
  expect(s.ledger.completeCandidateProvider).not.toHaveBeenCalled();
  expect(s.ledger.finish).not.toHaveBeenCalled();
  expect((s.budgetTelemetry.recordPostResponseProviderBudget as jest.Mock).mock.calls.some(call=>call[0]==='CANDIDATE_PROVIDER')).toBe(false);
 });
 it('persists the exact typed candidate plan, runs the one atomic persistence command, and hands generation Confidence to the managed batch',async()=>{const s=eligibleRun(setup({confidence:confidenceBatchEffect([confidenceReceipt(durableCandidate().hypothesisId,1,evaluationId.first),confidenceReceipt(secondId,2,evaluationId.second)])}));s.extraction.extract.mockResolvedValue({status:'AUTHORIZED',intent:intent()} as never);const plan={code:'VALIDATED_CANDIDATES' as const,candidates:[durableCandidate(),durableCandidate({hypothesisId:secondId,statement:'A second distinct statement.'})],subjectGroundings:[{hypothesisId:durableCandidate().hypothesisId,handles:[]},{hypothesisId:secondId,handles:[]}]};s.enrichment.generateHypothesisCandidatePlan.mockResolvedValue(plan as never);await s.service.dispatch(JSON.stringify(event()));
 ...
 describe('HIM Runtime Consumption v1 - fresh-path HIM before Candidate claim',()=>{
  ...
  it('reads canonical HIM BEFORE claiming CANDIDATE_PROVIDER and passes the minimized context into the one provider call',async()=>{
   ...
   expect(s.enrichment.generateHypothesisCandidatePlan).toHaveBeenCalledTimes(1);
   expect(s.enrichment.generateHypothesisCandidatePlan).toHaveBeenCalledWith(expect.anything(),expect.anything(),expect.anything(),himContext(),expect.objectContaining({executionId:id.execution}));
 ...
 describe('QAN-AUD-06 - managed generation Confidence batch',()=>{
  ...
  it('keeps the fresh execution ordering Memory -> Association -> Update -> Eligibility -> Intent -> Candidate -> Persistence -> managed Confidence -> terminal',async()=>{
   ...
   s.enrichment.generateHypothesisCandidatePlan.mockResolvedValue({code:'VALIDATED_CANDIDATES',candidates:[durableCandidate()],subjectGroundings:[{hypothesisId:durableCandidate().hypothesisId,handles:[]}]} as never);
   await s.service.dispatch(JSON.stringify(event()));
