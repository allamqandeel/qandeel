...
const completed = (over: Partial<Record<string, unknown>> = {}) => ({
  result_code: 'AUTHORIZED_COMMANDS', result_reference: null, result_commands: [command()], ...over,
});
...
describe('recoverAssociationResult', () => {
  it('recovers a valid NO_ASSOCIATION deterministically', () => {
    expect(recoverAssociationResult({ result_code: 'NO_ASSOCIATION', result_reference: null, result_commands: null }, EVIDENCE))
  ...
  it('recovers the exact durable command batch preserving every field', () => {
    ...
    expect(recoverAssociationResult(completed({ result_commands: commands }), EVIDENCE)).toEqual({
  ...
  it.each([
    ['null legacy result', { result_code: null, result_reference: null, result_commands: null }],
    ['unknown result code', { result_code: 'PARTIAL', result_reference: null, result_commands: null }],
    ['NO_ASSOCIATION with commands', { result_code: 'NO_ASSOCIATION', result_reference: null, result_commands: [command()] }],
    ['NO_ASSOCIATION with reference', { result_code: 'NO_ASSOCIATION', result_reference: EVIDENCE, result_commands: null }],
    ['AUTHORIZED with a stray reference', completed({ result_reference: EVIDENCE })],
    ['AUTHORIZED with an empty batch', completed({ result_commands: [] })],
    ['AUTHORIZED with a null batch', completed({ result_commands: null })],
    ['AUTHORIZED over the bound', completed({ result_commands: Array.from({ length: MAX_DURABLE_ASSOCIATION_COMMANDS + 1 }, (_u, index) => command({ hypothesisId: `30000000-0000-4000-8000-00000000000${index + 1}` })) })],
    ['duplicate hypothesis target', completed({ result_commands: [command({ hypothesisId: HYP_A }), command({ hypothesisId: HYP_A, evidenceRole: 'CONTRADICTING' })] })],
    ['wrong evidence identity', completed({ result_commands: [command({ evidenceId: OTHER_EVIDENCE })] })],
    ['extra command field', completed({ result_commands: [{ ...command(), extra: true }] })],
    ['missing command field', completed({ result_commands: [{ hypothesisId: HYP_A, expectedVersion: 1, evidenceId: EVIDENCE }] })],
    ['non-integer expected version', completed({ result_commands: [command({ expectedVersion: 1.5 })] })],
    ['zero expected version', completed({ result_commands: [command({ expectedVersion: 0 })] })],
    ['bad hypothesis uuid', completed({ result_commands: [command({ hypothesisId: 'nope' })] })],
    ['bad evidence reference', completed({ result_commands: [command({ evidenceId: 'memory:nope' })] })],
    ['invalid evidence role', completed({ result_commands: [command({ evidenceRole: 'NEUTRAL' })] })],
    ['non-array batch', completed({ result_commands: { hypothesisId: HYP_A } as unknown as unknown[] })],
