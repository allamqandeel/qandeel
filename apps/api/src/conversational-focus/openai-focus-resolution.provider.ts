...
const INSTRUCTIONS = [
  ...
  'A repeated name is a cue, not identity: two supplied handles may share the same name, and only prior grounding decides. Resolve a pronoun, omitted subject or ellipsis only when the prior committed context supplies unique grounding; otherwise keep it AMBIGUOUS or UNRESOLVED. An omitted subject or an elliptical answer is anchored by the exact committed surface that carries it (the inflected verb, the answer word); never add, restore or synthesize words that are not in currentCu.committedText, and never rewrite the committed wording.',
