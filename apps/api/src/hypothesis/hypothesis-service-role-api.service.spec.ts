...
const SENTINEL = 'SENTINEL_HYPOTHESIS_SERVICE_ROLE_KEY';

describe('HypothesisServiceRoleApiService', () => {
  const saved = { url: process.env.SUPABASE_URL, key: process.env.SUPABASE_SERVICE_ROLE_KEY };
  let service: HypothesisServiceRoleApiService;
  ...
  beforeEach(() => {
    ...
    service = new HypothesisServiceRoleApiService();
  });
  ...
  it('calls the named RPC with service-role authority and never a caller token', async () => {
    await service.rpc('server_create_hypothesis_v1', { p_user_id: 'user-a' });
    const [url, init] = (fetch as jest.Mock).mock.calls[0] as [string, RequestInit];
    expect(url).toBe('https://database.invalid/rest/v1/rpc/server_create_hypothesis_v1');
  ...
  it.each([
    ...
    unset();
    await expect(service.rpc('server_create_hypothesis_v1', {})).rejects.toBeInstanceOf(ServiceUnavailableException);
  ...
  it('sanitizes transport and status failures without leaking the service-role key', async () => {
    ...
    const transport = await service.rpc('server_create_hypothesis_v1', {}).catch((error: Error) => error);
    ...
    const rejected = await service.rpc('server_create_hypothesis_v1', {}).catch((error: Error) => error);
  ...
  it('exposes no caller-token parameter and no authenticated fallback', () => {
    ...
    // that documents it.
    const source = readFileSync(join(__dirname, 'hypothesis-service-role-api.service.ts'), 'utf8')
      .replace(/\/\/[^\n]*/gu, '');
    ...
    expect(new HypothesisServiceRoleApiService().rpc).toHaveLength(2);
  });
});
