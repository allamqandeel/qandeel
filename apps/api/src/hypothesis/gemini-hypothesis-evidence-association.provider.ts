...
function validSnapshot(value: HypothesisEvidenceAssociationSnapshot): boolean { if (!value || value.contractVersion !== HYPOTHESIS_EVIDENCE_ASSOCIATION_CONTRACT_VERSION || value.maxAssociationCount !== MAX_FRESH_EVIDENCE_ASSOCIATIONS || !validEvidence(value.freshEvidence) || !Array.isArray(value.candidateHypotheses) || value.candidateHypotheses.length > MAX_ASSOCIATION_HYPOTHESIS_CANDIDATES) return false; const ids = value.candidateHypotheses.map((candidate) => candidate.hypothesisId); return new Set(ids).size === ids.length && value.candidateHypotheses.every(validCandidate) && value.candidateHypotheses.reduce((total, candidate) => total + candidate.statement.length + candidate.scope.length + candidate.assumptions.reduce((sum, item) => sum + item.length, 0) + candidate.disconfirmingConditions.reduce((sum, item) => sum + item.length, 0), 0) <= MAX_ASSOCIATION_HYPOTHESIS_STRING_CHARACTERS; }
