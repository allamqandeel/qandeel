...
describe('MemoryRuntimeService', () => {
  ...
  beforeEach(() => {
    repository = {
      create: jest.fn(), find: jest.fn(), listActiveForUser: jest.fn(), markDeleted: jest.fn(), supersede: jest.fn(),
  ...
  it('creates an owned USER_STATED ACTIVE memory with a UUID', async () => {
    repository.create.mockImplementation(async (id, userId, input) => record({ id, user_id: userId, status: input.status }));
    const created = await runtime.create('user-a', base);
    expect(created.id).toMatch(/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/iu);
    expect(repository.create).toHaveBeenCalledWith(created.id, 'user-a', expect.objectContaining({ status: 'ACTIVE' }));
  });

  it.each(MEMORY_TYPES)('accepts canonical type %s', async (type) => {
    repository.create.mockResolvedValue(record({ type }));
    await expect(runtime.create('user-a', { ...base, type })).resolves.toBeDefined();
  });
  ...
  it.each(MEMORY_SOURCES)('accepts canonical source %s', async (source) => {
    ...
    await expect(runtime.create('user-a', { ...base, source })).resolves.toBeDefined();
  });

  it('rejects invalid types and sources', async () => {
    await expect(runtime.create('user-a', { ...base, type: 'TRANSCRIPT' as never })).rejects.toBeInstanceOf(BadRequestException);
    await expect(runtime.create('user-a', { ...base, source: 'MODEL_GUESSED' as never })).rejects.toBeInstanceOf(BadRequestException);
  });
  ...
  it.each([
    ...
    await expect(runtime.create('user-a', { ...base, [field]: value })).rejects.toBeInstanceOf(BadRequestException);
  });
  ...
  it('defaults SYSTEM_DERIVED to PENDING_CONFIRMATION and rejects ACTIVE', async () => {
    ...
    await runtime.create('user-a', { ...base, source: 'SYSTEM_DERIVED' });
    expect(repository.create).toHaveBeenCalledWith(expect.anything(), 'user-a', expect.objectContaining({ status: 'PENDING_CONFIRMATION' }));
    await expect(runtime.create('user-a', { ...base, source: 'SYSTEM_DERIVED', status: 'ACTIVE' })).rejects.toBeInstanceOf(BadRequestException);
  });
  ...
  it('allows a future expiration and rejects invalid or elapsed expiration', async () => {
    ...
    await expect(runtime.create('user-a', { ...base, type: 'TEMPORARY_STATE', expiresAt: '2999-01-01T00:00:00.000Z' })).resolves.toBeDefined();
    await expect(runtime.create('user-a', { ...base, expiresAt: 'invalid' })).rejects.toBeInstanceOf(BadRequestException);
    await expect(runtime.create('user-a', { ...base, expiresAt: '2000-01-01T00:00:00.000Z' })).rejects.toBeInstanceOf(BadRequestException);
  });
  ...
  it('fails closed when another user memory is invisible', async () => {
    ...
    await expect(runtime.markDeleted('user-a', 'memory-b')).rejects.toBeInstanceOf(NotFoundException);
  });
  ...
  it('delegates atomic supersession and preserves authenticated ownership', async () => {
    ...
    await expect(runtime.supersede('user-a', 'old', { ...base, content: 'I moved to Alexandria.' }))
  ...
  it('fails closed for cross-user supersession', async () => {
    ...
    await expect(runtime.supersede('user-a', 'memory-b', base)).rejects.toBeInstanceOf(NotFoundException);
  });

  it('never carries a caller credential into an authoritative Memory mutation', async () => {
    repository.create.mockResolvedValue(record());
    repository.markDeleted.mockResolvedValue(record({ status: 'DELETED' }));
    repository.supersede.mockResolvedValue(record({ id: 'successor', version: 2 }));
    await runtime.create('user-a', base);
    await runtime.markDeleted('user-a', 'memory-a');
    await runtime.supersede('user-a', 'old', base);
    for (const call of [
      ...repository.create.mock.calls, ...repository.markDeleted.mock.calls, ...repository.supersede.mock.calls,
    ]) expect(call).not.toContain('token-a');
    // The mutation signatures expose no access-token parameter at all.
    expect(runtime.create).toHaveLength(2);
    expect(runtime.markDeleted).toHaveLength(2);
    expect(runtime.supersede).toHaveLength(3);
  });
});
