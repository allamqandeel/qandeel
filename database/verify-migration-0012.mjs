...
try{
 ...
 if(state.rows.filter(x=>x.calculation_status==='CALIBRATED').map(x=>x.metric_key).sort().join()!=='hse.attention,hse.energy,hse.motivation,hse.self-confidence'||state.rows.filter(x=>x.calculation_status==='UNCALIBRATED').length!==13)throw new Error('Expected four calibrated HSE metrics');
