-- HRS Emotional Safety Measurement & Calibration v1 (HIM Expansion metric
-- 13/17 - the fourth and final HRS metric, bound to the reusable owned
-- RELATIONSHIP measurement-target substrate established by 0043 and reused
-- unchanged by 0044).
--
...
-- insecurity, mistake, limit, or disagreement - without reasonably
-- expecting that the vulnerability itself will be met with humiliating,
-- contemptuous, punitive, retaliatory, or weaponizing interpersonal
-- consequences. The measured object is the user's current perceived
-- interpersonal safety of emotional exposure. It is subjective and
-- relationship-specific: it is NEVER an objective classification of the
-- other person or the relationship. It is NOT physical, sexual, financial,
-- or medical safety, imminent-danger assessment, an abuse,
-- coercive-control, harassment, manipulation, gaslighting, or dangerousness
-- classifier, objective partner safety, relationship health or
-- satisfaction, love or closeness, attachment style or security, global or
-- social anxiety, conflict frequency or absence of conflict, politeness or
-- kindness, "never being challenged", lack of accountability,
-- hrs.relationship-trust, hrs.communication, hrs.repair, stay/leave
-- authority, or a clinical/diagnostic construct. A higher score means only
-- more self-reported emotional-openness safety in this relationship - never
-- proof the relationship or person is objectively safe or healthy - and a
-- lower score is never proof of abuse or danger. The word "Safety" in the
-- metric name grants this metric no system Safety authority: the existing
-- Safety Runtime remains separate and independently authoritative for
-- actual safety handling, and nothing here calls, suppresses, replaces, or
-- bypasses it, and no executable path converts a numeric value into any
-- safety, abuse, danger, or stay/leave verdict.
--
-- When perceived openness safety differs substantially by what would be
-- disclosed, TOO_VULNERABILITY_DEPENDENT_TO_RATE fails to UNASSESSED/NULL
-- instead of collapsing disclosure kinds into a misleading scalar (no
-- Emotional Safety subdomains exist in v1 and nothing is averaged), and
-- INSUFFICIENT_BASIS_TO_JUDGE fails to UNASSESSED/NULL when a new or sparse
-- relationship has produced too little meaningful emotional exposure -
-- missing basis is neither high nor low safety, never zero, and never a
-- midpoint.
--
-- The metric is an exactly-one-owned-RELATIONSHIP direct structured current
-- at-report appraisal with a NULL temporal window pair (no seven-day HBS
...
-- stays expressible: high Trust may coexist with low Emotional Safety, low
-- Communication with high Emotional Safety, and high Repair with low
-- Emotional Safety - no formula or constraint forces HRS metrics to
-- correlate, and no composite, inverse, or "relationship health" value
-- exists. The Foundation semantic mapping deliberately remains UNRESOLVED
-- with a NULL semantic type.
--
-- Substrate changes are explicit-union only, exactly as 0040-0044 did: the
...
-- the exact canonical 0044 definition and gain exactly the Emotional Safety
-- branch. The him_measurement_targets and him_measurement_events
-- context-kind unions already carry RELATIONSHIP from 0043 and are
-- untouched. The five HSE metrics, the four HBS metrics, and the three
-- prior HRS metrics remain semantically unchanged, every HGS metric remains
-- uncalibrated, and migrations 0001-0044 remain byte-exact unchanged. This
-- completes HRS measurement; HRS Runtime Consumption stays a later,
-- separately reviewed contract.

BEGIN;

-- 1. Response vocabulary stays an explicit per-family union: the exact
--    0044 HSE, Avoidance, Consistency, Initiative, Reflection, Relationship
--    Trust, Communication, and Repair vocabularies are preserved
--    byte-for-byte and exactly one Emotional Safety openness-safety
--    vocabulary is added. TOO_VULNERABILITY_DEPENDENT_TO_RATE belongs to
--    Emotional Safety only - never a permissive "any text" or shared-code
--    check.
...
-- 2. Structured observation contract: the exact 0044 union plus exactly one
--    Emotional Safety RELATIONSHIP branch with the exact server-derived
...
-- 3. Structured binding contract: the exact 0044 union plus exactly one
--    Emotional Safety RELATIONSHIP binding branch.
...
--    production model, one exactly-ten-basis approval, and exactly one
--    ACTIVE RELATIONSHIP binding for hrs.emotional-safety. Nothing is
--    shared with Trust, Communication, or Repair even though all four use
--    the same 1-5 numeric shape. No founder questionnaire, external
--    validation, or clinical validation is claimed.
INSERT INTO public.him_scale_contracts VALUES('45000000-0000-4000-8000-000000000001','hrs.emotional-safety.openness-safety-5.v1',1,'ORDINAL',true,false,false,ARRAY[1,2,3,4,5],'{"VERY_LOW":1,"LOW":2,"MODERATE":3,"HIGH":4,"VERY_HIGH":5}'::jsonb,'2026-08-27T00:00:00Z');
...
VALUES('45000000-0000-4000-8000-000000000002','hrs.emotional-safety.direct-structured-current-emotional-openness-safety',1,'hrs.emotional-safety',1,'CALIBRATED','PRODUCTION','QANDEEL_HIM_GOVERNANCE','HIM_EXPANSION_HRS_EMOTIONAL_SAFETY_MEASUREMENT_MODEL_V1','DIRECT_STRUCTURED_RELATIONSHIP_BOUND_CURRENT_EMOTIONAL_OPENNESS_SAFETY_REPORT','hrs.emotional-safety.openness-safety-5.v1','{"required":["measurementObservation","relationshipTarget"]}'::jsonb,'FIRST_CLASS_RELATIONSHIP_BOUND_HIM_MEASUREMENT_OBSERVATION_V1',ARRAY['RELATIONSHIP'],'UNASSESSED','UNASSESSED_PRESERVE_CONFLICT','UNRESOLVED_METRIC_CONFIDENCE','hrs-emotional-safety-direct-structured-relationship-bound-v1','2026-08-27T00:00:00Z','2026-08-27T00:00:00Z');
...
VALUES('45000000-0000-4000-8000-000000000003','qandeel.him.emotional-safety.foundation-approval',1,'QANDEEL_FOUNDATION_GOVERNANCE',1,'hrs.emotional-safety.direct-structured-current-emotional-openness-safety',1,'["HRS_EMOTIONAL_SAFETY_CURRENT_PERCEIVED_SAFETY_FOR_EMOTIONAL_OPENNESS","DIRECT_STRUCTURED_REPORT","RELATIONSHIP_BOUND_ONLY","EXPERIENCE_GROUNDED_CURRENT_APPRAISAL_NULL_WINDOW","ORDINAL_OPENNESS_SAFETY_5","VULNERABILITY_DEPENDENCE_AND_INSUFFICIENT_BASIS_FAIL_TO_UNASSESSED","EMOTIONAL_SAFETY_NOT_TRUST_COMMUNICATION_REPAIR_OR_OBJECTIVE_ABUSE_SAFETY","DETERMINISTIC_CALCULATION","CORRECTION_CURRENTNESS_IDEMPOTENCY_CONCURRENCY","SECURITY_BINDING_NO_EXTERNAL_OR_CLINICAL_VALIDATION_CLAIM"]'::jsonb,false,'2026-08-27T00:00:00Z','HIM_EXPANSION_HRS_EMOTIONAL_SAFETY_CANONICAL_APPROVAL');
...
--    NULL temporal window pair: Emotional Safety is an at-report
...
--    touch the shared HSE helpers or any sibling HBS/HRS function, and the
--    existing create_him_relationship_measurement_target_v1 RPC from 0043
--    is reused unchanged - no new target authority exists.
CREATE FUNCTION public.create_hrs_emotional_safety_measurement_v1(p_target_context_id uuid,p_response_code text,p_client_reported_at_untrusted timestamptz) RETURNS public.him_measurement_observations LANGUAGE plpgsql SECURITY DEFINER SET search_path='' AS $body$DECLARE u uuid:=auth.uid();e uuid:=gen_random_uuid();canonical_now timestamptz:=clock_timestamp();target public.him_measurement_targets;o public.him_measurement_observations;BEGIN
 ...
 IF NOT FOUND THEN RAISE EXCEPTION 'Unknown, cross-user, or unsupported Emotional Safety RELATIONSHIP target' USING ERRCODE='42501';END IF;
 IF p_response_code<>ALL(ARRAY['VERY_LOW','LOW','MODERATE','HIGH','VERY_HIGH','TOO_VULNERABILITY_DEPENDENT_TO_RATE','INSUFFICIENT_BASIS_TO_JUDGE','NOT_SURE']) THEN RAISE EXCEPTION 'Invalid Emotional Safety response' USING ERRCODE='22023';END IF;
 ...
 VALUES(gen_random_uuid(),u,e,'hrs.emotional-safety',1,'hrs.emotional-safety.direct-relationship-bound-emotional-openness-safety-report',1,'hrs.emotional-safety.openness-safety-5.v1',1,target.context_kind,target.id::text,p_response_code,canonical_now,p_client_reported_at_untrusted,'ar-EG','DIRECT_STRUCTURED_USER_REPORT','QANDEEL_HRS_EMOTIONAL_SAFETY_MEASUREMENT_V1',canonical_now,target.display_text,target.context_kind,target.id) RETURNING * INTO o;RETURN o;END$body$;

-- 8. Emotional Safety correction: same measurement event, same exact
...
--    serialized in the dedicated Emotional Safety namespace, one correction
...
CREATE FUNCTION public.correct_hrs_emotional_safety_measurement_v1(p_supersedes_observation_id uuid,p_response_code text,p_client_reported_at_untrusted timestamptz) RETURNS public.him_measurement_observations LANGUAGE plpgsql SECURITY DEFINER SET search_path='' AS $body$DECLARE u uuid:=auth.uid();canonical_now timestamptz:=clock_timestamp();prior public.him_measurement_observations;o public.him_measurement_observations;BEGIN
 PERFORM pg_advisory_xact_lock(hashtextextended('hrs.emotional-safety.observation:'||p_supersedes_observation_id::text,0));
 SELECT * INTO prior FROM public.him_measurement_observations WHERE id=p_supersedes_observation_id AND user_id=u AND metric_key='hrs.emotional-safety';
...
-- 9. Deterministic Emotional Safety calculation. Dedicated to
--    hrs.emotional-safety and structurally unable to score any sibling HRS,
--    HBS, or HSE observation: the metric filter, instrument, scale, and
--    binding join are all Emotional-Safety-exact, and no sibling metric,
--    Memory, Evidence, conversation text, or Safety Runtime state is ever
--    read - the relationship target label is verified as an exact
--    server-derived binding artifact, never interpreted semantically. It
--    re-verifies the exact owned RELATIONSHIP target, requires the event
--    window pair to be NULL, resolves the exact ACTIVE binding, and is
--    idempotent for the exact observation+binding pair. The snapshot
--    preserves semantic_mapping_status='UNRESOLVED' with a NULL semantic
--    type, carries a NULL temporal window, and stays
--    confidence-UNASSESSED. TOO_VULNERABILITY_DEPENDENT_TO_RATE,
--    INSUFFICIENT_BASIS_TO_JUDGE, and NOT_SURE map to UNASSESSED/NULL -
--    never zero, never a midpoint, never low safety, and never high safety.
--    The numeric value is never converted into any safety, abuse, danger,
--    or stay/leave verdict, and no verdict field exists.
CREATE FUNCTION public.calculate_hrs_emotional_safety_measurement_v1(p_observation_id uuid) RETURNS public.him_metric_snapshots LANGUAGE plpgsql SECURITY DEFINER SET search_path='' AS $body$DECLARE u uuid:=auth.uid();o public.him_measurement_observations;e public.him_measurement_events;b public.him_canonical_model_bindings;r public.him_calculation_results;score double precision;state text;next_version integer;s public.him_metric_snapshots;BEGIN
 PERFORM pg_advisory_xact_lock(hashtextextended('hrs.emotional-safety.observation:'||p_observation_id::text,0));
 SELECT * INTO o FROM public.him_measurement_observations WHERE id=p_observation_id AND user_id=u AND metric_key='hrs.emotional-safety';
 IF NOT FOUND OR EXISTS(SELECT 1 FROM public.him_measurement_observations x WHERE x.supersedes_observation_id=o.id) THEN RAISE EXCEPTION 'Unknown, cross-user, cross-metric, or superseded Emotional Safety observation' USING ERRCODE='42501';END IF;
 IF NOT(o.context_kind='RELATIONSHIP' AND o.target_context_kind=o.context_kind AND o.target_context_id::text=o.context_id AND EXISTS(SELECT 1 FROM public.him_measurement_targets t WHERE t.id=o.target_context_id AND t.user_id=o.user_id AND t.context_kind='RELATIONSHIP' AND t.display_text=o.target_label)) THEN RAISE EXCEPTION 'Emotional Safety target/context mismatch' USING ERRCODE='22023';END IF;
 SELECT * INTO e FROM public.him_measurement_events WHERE id=o.measurement_event_id AND user_id=o.user_id;
 IF e.id IS NULL OR e.observation_window_start IS NOT NULL OR e.observation_window_end IS NOT NULL THEN RAISE EXCEPTION 'Emotional Safety observation window must remain NULL' USING ERRCODE='22023';END IF;
 ...
 IF NOT FOUND THEN RAISE EXCEPTION 'No exact active canonical Emotional Safety binding' USING ERRCODE='22023';END IF;
...
--     hrs.emotional-safety so explicit corrections can never leave a stale
--     superseded value as "latest current". The five HSE routes, the four
--     HBS routes, and the three prior HRS routes are preserved exactly, and
--     no HGS metric is routed - all four HGS metrics remain uncalibrated
--     and unobserved.
CREATE OR REPLACE VIEW public.him_current_structured_measurements WITH(security_invoker=true)AS SELECT s.* FROM public.him_measurement_observations o JOIN public.him_metric_snapshots s ON s.measurement_observation_id=o.id WHERE o.metric_key=ANY(ARRAY['hse.energy','hse.motivation','hse.attention','hse.self-confidence','hse.stress','hbs.avoidance','hbs.consistency','hbs.initiative','hbs.reflection','hrs.relationship-trust','hrs.communication','hrs.repair','hrs.emotional-safety'])AND NOT EXISTS(SELECT 1 FROM public.him_measurement_observations newer WHERE newer.supersedes_observation_id=o.id)AND NOT EXISTS(SELECT 1 FROM public.him_energy_calculation_supersessions x WHERE x.snapshot_id=s.id);

REVOKE ALL ON FUNCTION public.create_hrs_emotional_safety_measurement_v1(uuid,text,timestamptz),public.correct_hrs_emotional_safety_measurement_v1(uuid,text,timestamptz),public.calculate_hrs_emotional_safety_measurement_v1(uuid) FROM PUBLIC,anon;
GRANT EXECUTE ON FUNCTION public.create_hrs_emotional_safety_measurement_v1(uuid,text,timestamptz),public.correct_hrs_emotional_safety_measurement_v1(uuid,text,timestamptz),public.calculate_hrs_emotional_safety_measurement_v1(uuid) TO authenticated;

-- 11. One-time migration-phase inventory invariant: exactly thirteen
--     calibrated (five HSE, four HBS, and the four HRS metrics) and exactly
--     four uncalibrated (the four HGS metrics), Emotional Safety keeps HRS
--     ownership with an unresolved NULL semantic mapping, its exact scale
--     and input contract, and no dependency edges or consumers, the prior
--     HSE/HBS metrics and the three prior HRS metrics remain calibrated and
--     semantically unchanged, and all four HGS metrics remain uncalibrated.
--     This completes the HRS measurement family. The 13/4 phase check runs
--     at migration time only - historical verifiers must never freeze it as
--     a permanent ceiling, and nothing here asserts that a later migration
--     can never exist. The temporal-comparability source (0017) and
--     Intelligence Snapshot v1 (0018) are untouched by this migration:
--     hrs.emotional-safety is deliberately NOT ELIGIBLE for either in v1,
--     and measurement completion grants no runtime-consumption authority.
DO $$BEGIN
 IF (SELECT count(*) FROM public.him_metric_definitions WHERE calculation_status='CALIBRATED')<>13
  OR (SELECT count(*) FROM public.him_metric_definitions WHERE calculation_status='UNCALIBRATED')<>4
  ...
  OR EXISTS(SELECT 1 FROM public.him_metric_definitions WHERE metric_key=ANY(ARRAY['hgs.self-awareness','hgs.resilience','hgs.purpose-alignment','hgs.habit-strength']) AND calculation_status<>'UNCALIBRATED')
 THEN RAISE EXCEPTION 'Thirteen calibrated / four uncalibrated Emotional Safety activation invariant failed';END IF;
END$$;
COMMIT;
