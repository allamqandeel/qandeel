-- HBS Reflection Measurement & Calibration v1 (HIM Expansion metric 9/17).
--
-- Activates exactly one metric: hbs.reflection@1 becomes a CALIBRATED, exact
-- context-bound, direct structured measurement while its Foundation semantic
-- mapping deliberately remains UNRESOLVED with a NULL semantic type. The
-- construct is narrow: context-bound deliberate reflective engagement - at
-- the time of report, to what extent the user has intentionally stepped back
-- to examine ONE exact owned SITUATION or CONVERSATION_SESSION, including
-- what happened, their own actions or assumptions, and possible alternative
-- interpretations, learning, or adjustment. It measures the reflective
-- behavior/process, never whether the user reached a correct insight. It is
-- NOT a permanent trait, rumination/overthinking, hgs.self-awareness, insight
-- accuracy, emotional intelligence, learning outcome, behavior change,
-- accountability/guilt, Recommendation readiness, clinical functioning,
-- LLM-derived signal, Trend v1 metric, or Intelligence Snapshot v1 input.
-- Direction is not valence: more reflection is never automatically better.
--
-- Reflection deliberately does NOT reuse the seven-day HBS period model of
-- Avoidance/Consistency/Initiative: it is one exact context-bound at-report
-- reflective-engagement assessment. Its measurement events carry a NULL
-- temporal window pair (the durable 0040 window columns stay NULL, exactly
-- like the HSE right-now events), no caller-selected window exists, and the
-- server-authoritative report time is the measurement time.
--
-- Substrate changes are explicit-union only, exactly as 0040/0041 did to the
-- 0016 contracts: the per-family response vocabulary, observation contract,
-- binding contract, binding-validation function, and current structured read
-- gain exactly the Reflection branches. The five HSE metrics and the three
-- seven-day HBS metrics remain semantically unchanged, and migrations
-- 0001-0041 remain byte-exact unchanged.

BEGIN;

-- 1. Response vocabulary stays an explicit per-family union: the exact HSE,
--    Avoidance, Consistency, and Initiative vocabularies are preserved
--    byte-for-byte and exactly one Reflection engagement vocabulary is added.
--    NO_MEANINGFUL_OPPORTUNITY_TO_REFLECT belongs to Reflection only - never
...
-- 2. Structured observation contract: the exact 0041 union plus exactly two
--    Reflection branches - a CONVERSATION_SESSION branch with mandatory NULL
--    target fields (no fake measurement targets for sessions) and a
--    SITUATION branch with the exact server-derived target shape.
...
-- 3. Structured binding contract: the exact 0041 union plus exactly one
--    Reflection SITUATION/CONVERSATION_SESSION binding branch.
...
-- 4. Governance artifacts: one ordinal engagement scale, one calibrated
--    production model, one exactly-ten-basis approval, and exactly the two
--    ACTIVE SITUATION/CONVERSATION_SESSION bindings. No founder
--    questionnaire, external validation, or clinical validation is claimed.
INSERT INTO public.him_scale_contracts VALUES('42000000-0000-4000-8000-000000000001','hbs.reflection.engagement-5.v1',1,'ORDINAL',true,false,false,ARRAY[1,2,3,4,5],'{"NOT_AT_ALL":1,"A_LITTLE":2,"SOMEWHAT":3,"QUITE_A_BIT":4,"A_GREAT_DEAL":5}'::jsonb,'2026-08-27T00:00:00Z');
...
VALUES('42000000-0000-4000-8000-000000000002','hbs.reflection.direct-structured-context-bound-reflective-engagement',1,'hbs.reflection',1,'CALIBRATED','PRODUCTION','QANDEEL_HIM_GOVERNANCE','HIM_EXPANSION_HBS_REFLECTION_MEASUREMENT_MODEL_V1','DIRECT_STRUCTURED_CONTEXT_BOUND_REFLECTIVE_ENGAGEMENT_REPORT','hbs.reflection.engagement-5.v1','{"required":["measurementObservation","authorizedContext"]}'::jsonb,'FIRST_CLASS_AUTHORIZED_CONTEXT_HIM_MEASUREMENT_OBSERVATION_V1',ARRAY['SITUATION','CONVERSATION_SESSION'],'UNASSESSED','UNASSESSED_PRESERVE_CONFLICT','UNRESOLVED_METRIC_CONFIDENCE','hbs-reflection-direct-structured-context-bound-v1','2026-08-27T00:00:00Z','2026-08-27T00:00:00Z');
...
VALUES('42000000-0000-4000-8000-000000000003','qandeel.him.reflection.foundation-approval',1,'QANDEEL_FOUNDATION_GOVERNANCE',1,'hbs.reflection.direct-structured-context-bound-reflective-engagement',1,'["HBS_REFLECTION_CONTEXT_BOUND_DELIBERATE_REFLECTIVE_ENGAGEMENT","DIRECT_STRUCTURED_REPORT","SITUATION_SESSION_ONLY","MEANINGFUL_REFLECTION_OPPORTUNITY_BOUNDARY","ORDINAL_ENGAGEMENT_5","REFLECTION_NOT_RUMINATION_OR_INSIGHT_OUTCOME","DETERMINISTIC_CALCULATION","CORRECTION_CURRENTNESS_IDEMPOTENCY_CONCURRENCY","SECURITY_BINDING","NO_FOUNDER_EXTERNAL_OR_CLINICAL_VALIDATION_CLAIM"]'::jsonb,false,'2026-08-27T00:00:00Z','HIM_EXPANSION_HBS_REFLECTION_CANONICAL_APPROVAL');
...
('42000000-0000-4000-8000-000000000004','hbs.reflection',1,'SITUATION',1,'ACTIVE','hbs.reflection.direct-structured-context-bound-reflective-engagement',1,'hbs.reflection.direct-context-bound-reflective-engagement-report',1,'hbs.reflection.engagement-5.v1',1,'qandeel.him.reflection.foundation-approval',1,'2026-08-27T00:00:00Z'),
('42000000-0000-4000-8000-000000000005','hbs.reflection',1,'CONVERSATION_SESSION',1,'ACTIVE','hbs.reflection.direct-structured-context-bound-reflective-engagement',1,'hbs.reflection.direct-context-bound-reflective-engagement-report',1,'hbs.reflection.engagement-5.v1',1,'qandeel.him.reflection.foundation-approval',1,'2026-08-27T00:00:00Z');

-- 5. Definition activation: exactly the Reflection definition moves from
...
CREATE FUNCTION public.create_hbs_reflection_measurement_v1(p_context_kind text,p_context_id uuid,p_response_code text,p_client_reported_at_untrusted timestamptz) RETURNS public.him_measurement_observations LANGUAGE plpgsql SECURITY DEFINER SET search_path='' AS $body$DECLARE u uuid:=auth.uid();e uuid:=gen_random_uuid();canonical_now timestamptz:=clock_timestamp();target public.him_measurement_targets;o public.him_measurement_observations;BEGIN
 ...
 VALUES(gen_random_uuid(),u,e,'hbs.reflection',1,'hbs.reflection.direct-context-bound-reflective-engagement-report',1,'hbs.reflection.engagement-5.v1',1,p_context_kind,p_context_id::text,p_response_code,canonical_now,p_client_reported_at_untrusted,'ar-EG','DIRECT_STRUCTURED_USER_REPORT','QANDEEL_HBS_REFLECTION_MEASUREMENT_V1',canonical_now,target.display_text,target.context_kind,target.id) RETURNING * INTO o;RETURN o;END$body$;

-- 7. Reflection correction: same measurement event, same exact context
--    (SITUATION or session - a correction can never switch context), same
--    immutable NULL event window, advisory-lock serialized in its own
--    namespace, one correction per observation, and the prior calculated
--    result/current snapshot is superseded through the existing hardened
--    supersession ledger. A correction changes the response, nothing else.
CREATE FUNCTION public.correct_hbs_reflection_measurement_v1(p_supersedes_observation_id uuid,p_response_code text,p_client_reported_at_untrusted timestamptz) RETURNS public.him_measurement_observations LANGUAGE plpgsql SECURITY DEFINER SET search_path='' AS $body$DECLARE u uuid:=auth.uid();canonical_now timestamptz:=clock_timestamp();prior public.him_measurement_observations;o public.him_measurement_observations;BEGIN
 PERFORM pg_advisory_xact_lock(hashtextextended('hbs.reflection.observation:'||p_supersedes_observation_id::text,0));
 SELECT * INTO prior FROM public.him_measurement_observations WHERE id=p_supersedes_observation_id AND user_id=u AND metric_key='hbs.reflection';
...
-- 8. Deterministic Reflection calculation. Dedicated to hbs.reflection and
--    structurally unable to score any sibling HBS or HSE observation: the
--    metric filter, instrument, scale, and binding join are all
--    Reflection-exact, and no sibling metric, Self-awareness surface,
--    Memory, Evidence, or conversation text is ever read. It re-verifies
--    the exact SITUATION target or exact owned session, requires the event
...
--    type, carries a NULL temporal window, and stays confidence-UNASSESSED.
--    NO_MEANINGFUL_OPPORTUNITY_TO_REFLECT and NOT_SURE map to
--    UNASSESSED/NULL, never zero and never a low score.
CREATE FUNCTION public.calculate_hbs_reflection_measurement_v1(p_observation_id uuid) RETURNS public.him_metric_snapshots LANGUAGE plpgsql SECURITY DEFINER SET search_path='' AS $body$DECLARE u uuid:=auth.uid();o public.him_measurement_observations;e public.him_measurement_events;b public.him_canonical_model_bindings;r public.him_calculation_results;score double precision;state text;next_version integer;s public.him_metric_snapshots;BEGIN
 PERFORM pg_advisory_xact_lock(hashtextextended('hbs.reflection.observation:'||p_observation_id::text,0));
 SELECT * INTO o FROM public.him_measurement_observations WHERE id=p_observation_id AND user_id=u AND metric_key='hbs.reflection';
 IF NOT FOUND OR EXISTS(SELECT 1 FROM public.him_measurement_observations x WHERE x.supersedes_observation_id=o.id) THEN RAISE EXCEPTION 'Unknown, cross-user, cross-metric, or superseded Reflection observation' USING ERRCODE='42501';END IF;
 IF NOT((o.context_kind='CONVERSATION_SESSION' AND EXISTS(SELECT 1 FROM public.conversation_sessions c WHERE c.id::text=o.context_id AND c.user_id=o.user_id) AND o.target_label IS NULL AND o.target_context_kind IS NULL AND o.target_context_id IS NULL) OR (o.context_kind='SITUATION' AND o.target_context_kind=o.context_kind AND o.target_context_id::text=o.context_id AND EXISTS(SELECT 1 FROM public.him_measurement_targets t WHERE t.id=o.target_context_id AND t.user_id=o.user_id AND t.context_kind='SITUATION' AND t.display_text=o.target_label))) THEN RAISE EXCEPTION 'Reflection authoritative context mismatch' USING ERRCODE='22023';END IF;
 SELECT * INTO e FROM public.him_measurement_events WHERE id=o.measurement_event_id AND user_id=o.user_id;
 IF e.id IS NULL OR e.observation_window_start IS NOT NULL OR e.observation_window_end IS NOT NULL THEN RAISE EXCEPTION 'Reflection observation window must remain NULL' USING ERRCODE='22023';END IF;
 ...
 IF NOT FOUND THEN RAISE EXCEPTION 'No exact active canonical Reflection binding' USING ERRCODE='22023';END IF;
 ...
 score:=CASE o.response_code WHEN 'NOT_AT_ALL' THEN 1 WHEN 'A_LITTLE' THEN 2 WHEN 'SOMEWHAT' THEN 3 WHEN 'QUITE_A_BIT' THEN 4 WHEN 'A_GREAT_DEAL' THEN 5 ELSE NULL END;state:=CASE WHEN score IS NULL THEN 'UNASSESSED' ELSE 'ASSESSED' END;
...
-- 9. Current structured read: the supersession-aware view now also carries
--    hbs.reflection so explicit corrections can never leave a stale
--    superseded value as "latest current". The five HSE routes and the
--    three seven-day HBS routes are preserved exactly.
CREATE OR REPLACE VIEW public.him_current_structured_measurements WITH(security_invoker=true)AS SELECT s.* FROM public.him_measurement_observations o JOIN public.him_metric_snapshots s ON s.measurement_observation_id=o.id WHERE o.metric_key=ANY(ARRAY['hse.energy','hse.motivation','hse.attention','hse.self-confidence','hse.stress','hbs.avoidance','hbs.consistency','hbs.initiative','hbs.reflection'])AND NOT EXISTS(SELECT 1 FROM public.him_measurement_observations newer WHERE newer.supersedes_observation_id=o.id)AND NOT EXISTS(SELECT 1 FROM public.him_energy_calculation_supersessions x WHERE x.snapshot_id=s.id);

REVOKE ALL ON FUNCTION public.create_hbs_reflection_measurement_v1(text,uuid,text,timestamptz),public.correct_hbs_reflection_measurement_v1(uuid,text,timestamptz),public.calculate_hbs_reflection_measurement_v1(uuid) FROM PUBLIC,anon;
GRANT EXECUTE ON FUNCTION public.create_hbs_reflection_measurement_v1(text,uuid,text,timestamptz),public.correct_hbs_reflection_measurement_v1(uuid,text,timestamptz),public.calculate_hbs_reflection_measurement_v1(uuid) TO authenticated;

-- 10. One-time migration-phase inventory invariant: exactly nine calibrated
--     (five HSE plus hbs.avoidance, hbs.consistency, hbs.initiative, and
--     hbs.reflection), exactly eight uncalibrated, Reflection keeps HBS
--     ownership with an unresolved NULL semantic mapping, its exact scale
--     and input contract, and no dependency edges or consumers, and the
--     prior HSE/HBS metrics remain calibrated and semantically unchanged.
--     This 9/8 phase check runs at migration time only - historical
--     verifiers must never freeze it as a permanent ceiling. The
--     temporal-comparability source (0017) and Intelligence Snapshot v1
--     (0018) are untouched by this migration: hbs.reflection is
--     deliberately NOT ELIGIBLE for either in v1.
DO $$BEGIN
 IF (SELECT count(*) FROM public.him_metric_definitions WHERE calculation_status='CALIBRATED')<>9
  OR (SELECT count(*) FROM public.him_metric_definitions WHERE calculation_status='UNCALIBRATED')<>8
 ...
 THEN RAISE EXCEPTION 'Nine calibrated / eight uncalibrated Reflection activation invariant failed';END IF;
END$$;
COMMIT;
