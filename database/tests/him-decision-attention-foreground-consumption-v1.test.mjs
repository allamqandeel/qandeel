...
// QHIA-008 static contract. Freezes the 0057 migration's OWN authority
// contract: exactly one read-only, non-SECURITY-DEFINER, authenticated-only
// composition RPC that answers the Decision-bound hse.attention@1 question in
...
// 0054) - never by reimplementing, bypassing, or widening either, never by
// activating a second context kind or a second metric, and never by
// generalising QHIA-007's proven shape into an early QHIA-009 aggregator.
//
...
// task. Every assertion runs against the 0057 migration text itself (or this
// task's own wiring), never against the global schema or function namespace.
const root=new URL('../',import.meta.url);
const read=path=>readFileSync(new URL(path,root),'utf8');
const MIGRATION='0057_him_decision_attention_foreground_consumption_v1.sql';
...
const verifier=read('verify-migration-0057.mjs');
...
const FUNCTION_SIGNATURE='public.read_him_session_decision_attention_v1(uuid,uuid)';
...
const REQUIRED=[
 [/CREATE FUNCTION public\.read_him_session_decision_attention_v1\(p_user_id uuid,p_session_id uuid\)/,'the exact narrow two-parameter composition signature'],
 ...
 [/WHERE b\.context_kind='DECISION'/,'exactly the DECISION kind is filtered out of the authoritative binding result'],
 [/IF decisions IS NULL OR cardinality\(decisions\)=0 THEN/,'an absent ACTIVE Decision binding is answered deterministically'],
 [/'NO_ACTIVE_DECISION'::text/,'the deterministic unbound no-effect result exists'],
 [/'ACTIVE_DECISION_BOUND'::text/,'the bound result is explicitly discriminated'],
 [/IF cardinality\(decisions\)>1 THEN RAISE EXCEPTION 'Ambiguous ACTIVE Decision relevance binding'/,'more than one ACTIVE Decision fails closed instead of choosing one'],
 [/FROM public\.read_him_contextual_current_intelligence_batch_v1\(p_user_id,'DECISION',bound_decision::text,ARRAY\['hse\.attention'\],ARRAY\[1\]\) c/,'current intelligence is delegated to the QHIA-004 authority for exactly DECISION + hse.attention@1'],
 [/REVOKE ALL ON FUNCTION public\.read_him_session_decision_attention_v1\(uuid,uuid\) FROM PUBLIC,anon,authenticated,service_role;/,'every default privilege is revoked first'],
 [/GRANT EXECUTE ON FUNCTION public\.read_him_session_decision_attention_v1\(uuid,uuid\) TO authenticated;/,'authenticated is the only EXECUTE grantee'],
 ...
 [/'hse\.self-confidence'/,'the postcondition proves the dormant second Decision metric is absent from the installed definition'],
];
const FORBIDDEN=[
 ...
 [/CREATE OR REPLACE FUNCTION/,'no existing function is replaced: 0054, 0055 and 0056 are composed or left alone, never rewritten'],
...
const FORBIDDEN_IN_AUTHORITY=[
 [/'GOAL'|'SITUATION'|'RELATIONSHIP'|'CONVERSATION_SESSION'|'GLOBAL'/,'QHIA-008 activates no context kind other than DECISION'],
 [/hse\.energy|hse\.stress|hse\.motivation|hse\.self-confidence|hbs\.|hrs\.|hgs\./,'QHIA-008 activates no metric other than hse.attention: self-confidence stays dormant'],
 [/read_him_session_situation_stress_v1/,'the QHIA-007 composition is neither called, wrapped, nor generalised by this task'],
 ...
 [/LIMIT\s+1/i,'no latest/first/only-Decision fallback exists'],
...
function assertDecisionAttentionAuthorityContract(text){
 const start=text.lastIndexOf('DO $$DECLARE fn text');
 const authority=start>0?text.slice(0,start):text;
 for(const[pattern,property]of REQUIRED)if(!pattern.test(text))throw new Error(`QHIA-008 decision-attention authority contract violated: ${property}`);
 for(const[pattern,property]of FORBIDDEN)if(pattern.test(text))throw new Error(`QHIA-008 decision-attention authority contract violated: ${property}`);
 for(const[pattern,property]of FORBIDDEN_IN_AUTHORITY)if(pattern.test(authority))throw new Error(`QHIA-008 decision-attention authority contract violated: ${property}`);
}
...
function assertMigrationIdentity(names){
 ...
 assert.ok(migrations.includes(MIGRATION),'migration 0057 exists');
 ...
 assert.ok(migrations.indexOf(MIGRATION)>migrations.indexOf('0056_him_situation_stress_foreground_consumption_v1.sql'),'0057 orders after 0056');
 assert.ok(migrations.indexOf(MIGRATION)>migrations.indexOf('0055_him_session_context_binding_relevance_v1.sql'),'0057 orders after 0055');
 assert.ok(migrations.indexOf(MIGRATION)>migrations.indexOf('0054_him_contextual_current_intelligence_batch_read_v1.sql'),'0057 orders after 0054');
}

test('S1 - migration 0057 installs only the one narrow composition and satisfies the frozen authority contract',()=>{
 assertMigrationIdentity(readdirSync(new URL('migrations/',root)).filter(name=>name.endsWith('.sql')));
 assert.doesNotThrow(()=>assertDecisionAttentionAuthorityContract(executable),'the shipped migration satisfies the frozen contract');
...
test('S2 - hard authority rule: relevance and current intelligence are COMPOSED, never reimplemented',()=>{
 ...
 // The bound Decision is server-resolved and typed: the application never
 // supplies it and never picks between candidates.
 assert.match(executable,/bound_decision:=decisions\[1\];/,'the single ACTIVE Decision is taken only after the cardinality guard');
 assert.match(executable,/SELECT array_agg\(b\.context_id\) INTO decisions/,'the authoritative binding result is aggregated so its cardinality is explicit');
 // Exactly one delegated slot is requested.
 assert.equal((executable.match(/ARRAY\['hse\.attention'\]/g)??[]).length,1,'exactly one metric slot is requested');
 ...
 assert.match(executable,/QHIA-008 activates exactly DECISION \+ hse\.attention@1/,'the postcondition proves no other context or metric on the installed definition');
 assert.match(executable,/must be read-only/,'the postcondition proves the installed definition is non-mutating');
 // QHIA-007 is left exactly as migration 0056 installed it, and is required
 // to still be present rather than replaced by this task.
 assert.match(executable,/to_regprocedure\('public\.read_him_session_situation_stress_v1\(uuid,uuid\)'\) IS NULL THEN RAISE EXCEPTION/,'the QHIA-007 authority must remain installed');
 assert.doesNotMatch(authoritySlice,/read_him_session_situation_stress_v1/,'QHIA-008 neither calls nor wraps the QHIA-007 composition');
});

test('S3 - anti-vacuity: the real guard rejects every named drift fixture',()=>{
 const drifts=[
  ['a second context kind was activated',executable.replace("WHERE b.context_kind='DECISION'","WHERE b.context_kind=ANY(ARRAY['DECISION','SITUATION'])")],
  ['the dormant second Decision metric was activated',executable.replace("ARRAY['hse.attention']","ARRAY['hse.attention','hse.self-confidence']")],
  ['a generic caller-selected metric list appeared',executable.replace('CREATE FUNCTION public.read_him_session_decision_attention_v1(p_user_id uuid,p_session_id uuid)','CREATE FUNCTION public.read_him_session_decision_attention_v1(p_user_id uuid,p_session_id uuid,p_metric_keys text[])')],
  ['the callable surface gained a caller-supplied context id',executable.replace('CREATE FUNCTION public.read_him_session_decision_attention_v1(p_user_id uuid,p_session_id uuid)','CREATE FUNCTION public.read_him_session_decision_attention_v1(p_user_id uuid,p_session_id uuid,p_context_id uuid)')],
  ...
  ['the QHIA-004 authority was bypassed for the canonical latest authority',executable.replace("FROM public.read_him_contextual_current_intelligence_batch_v1(p_user_id,'DECISION',bound_decision::text,ARRAY['hse.attention'],ARRAY[1]) c","FROM public.read_him_latest_measurement_v1(p_user_id,'hse.attention',1,'DECISION',bound_decision::text) c")],
  ['an early QHIA-009 aggregation over the QHIA-007 composition appeared',executable.replace('bound_decision:=decisions[1];','bound_decision:=decisions[1];\n PERFORM * FROM public.read_him_session_situation_stress_v1(p_user_id,p_session_id);')],
  ['a latest-Decision fallback replaced the authoritative binding filter',executable.replace("WHERE b.context_kind='DECISION'",'ORDER BY b.created_at DESC LIMIT 1')],
  ['the unbound no-effect answer was removed',executable.replace("'NO_ACTIVE_DECISION'::text","'UNKNOWN'::text")],
  ['the ambiguity guard was removed',executable.replace("IF cardinality(decisions)>1 THEN RAISE EXCEPTION 'Ambiguous ACTIVE Decision relevance binding' USING ERRCODE='55000';END IF;",'')],
  ...
  ['authentication was re-implemented instead of delegated',executable.replace('DECLARE decisions uuid[];bound_decision uuid;','DECLARE decisions uuid[];bound_decision uuid;u uuid:=auth.uid();')],
  ['a mutation appeared in the read path',executable.replace('bound_decision:=decisions[1];','bound_decision:=decisions[1];\n UPDATE public.him_session_context_bindings SET status=status;')],
  ...
  ['the dormancy postcondition was removed',executable.replace("'hse.self-confidence',",'')],
 ];
 for(const[label,mutated]of drifts){
  assert.notEqual(mutated,executable,`the "${label}" mutation actually replaced its source text`);
  assert.throws(()=>assertDecisionAttentionAuthorityContract(mutated),/QHIA-008 decision-attention authority contract violated/,`the guard rejects: ${label}`);
 }
 // Positive control and formatting-insensitivity: cosmetic whitespace never
 // fails the guard.
 assert.doesNotThrow(()=>assertDecisionAttentionAuthorityContract(executable));
 ...
 assert.doesNotThrow(()=>assertDecisionAttentionAuthorityContract(reformatted),'formatting alone never fails the guard');
});
...
test('S4 - the guard creates no future ceiling and weakens no prior verification',()=>{
 ...
 assert.ok(!executable.includes(NEXT_MIGRATION_NUMBER),'0057 freezes no next-migration number');
 assert.ok(!verifier.includes(NEXT_MIGRATION_NUMBER),'the 0057 verifier freezes no next-migration number');
 ...
 for(const preserved of['verify-migration-0054.mjs','verify-migration-0055.mjs','verify-migration-0056.mjs'])assert.ok(files.includes(preserved),`the ${preserved} verifier is preserved`);
 const contracts=readdirSync(new URL('tests/',root));
 for(const preserved of['him-session-context-binding-relevance-v1.test.mjs','him-situation-stress-foreground-consumption-v1.test.mjs'])assert.ok(contracts.includes(preserved),`the ${preserved} contract is preserved`);
 for(const preserved of['0054_him_contextual_current_intelligence_batch_read_v1.sql','0055_him_session_context_binding_relevance_v1.sql','0056_him_situation_stress_foreground_consumption_v1.sql'])assert.ok(listing.includes(preserved),`migration ${preserved} is preserved`);
 // The QHIA-007 migration text itself is byte-identical to what 0056 shipped:
 // this task changes no prior authority.
...
test('the 0057 verifier proves the required live-schema scenarios on a real connection and stays non-destructive',()=>{
 for(const proof of[
  'to_regprocedure','pg_get_functiondef','has_function_privilege','prosecdef','provolatile','pronargs',
  'read_him_session_decision_attention_v1','set_him_session_context_binding_v1','clear_him_session_context_binding_v1',
  'read_him_contextual_current_intelligence_batch_v1','read_him_latest_measurement_v1','him_active_structured_binding_id',
  'create_him_attention_measurement_context','create_hse_attention_measurement','calculate_hse_attention_measurement',
  'create_hse_self_confidence_measurement','calculate_hse_self_confidence_measurement',
  'create_him_motivation_measurement_target','create_him_relationship_measurement_target_v1',
  'NO_ACTIVE_DECISION','ACTIVE_DECISION_BOUND','cleanupVerifierUsers',
 ...
 assert.match(verifier,/QHIA-008 must never read or return hse\.self-confidence/,'the dormancy of the second Decision metric is proven on real rows');
...
test('the 0057 verifier is wired after the 0056 Situation-stress verifier and before the downstream HIM consumption gates',()=>{
 const packageJson=JSON.parse(read('../package.json'));
 assert.match(packageJson.scripts['verify:him-decision-attention-foreground-consumption:integration'],/--env-file-if-exists=\.env database\/verify-migration-0057\.mjs/);
 const ci=read('../.github/workflows/api-ci.yml');
 const step=ci.indexOf('verify:him-decision-attention-foreground-consumption:integration');
 assert.ok(step>0,'CI runs the 0057 verifier');
