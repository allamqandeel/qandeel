...
test('keeps every initial metric uncalibrated without dependency edges',()=>{assert.equal((sql.match(/'UNCALIBRATED'/g)??[]).length,19);assert.equal((sql.match(/'UNCALIBRATED_NO_PRODUCTION_SCALE'/g)??[]).length,18);assert.equal((sql.match(/'UNRESOLVED_METRIC_CONFIDENCE_MODEL'/g)??[]).length,18);assert.equal((sql.match(/ARRAY\['HIF_PART_8_HUMAN_INTELLIGENCE_METRICS_V0_1'\],'\{\}'/g)??[]).length,17);});
...
test('migration fails on pre-existing or canonical metadata drift',()=>{assert.match(sql,/count\(\*\).*<>17/s);assert.match(sql,/Initial HIM catalog drift detected/);assert.match(sql,/cardinality\(dependency_ids\)<>0/);});
