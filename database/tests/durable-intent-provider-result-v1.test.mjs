...
test('the repository exposes a typed Intent completion and excludes typed effects from the generic one', () => {
  ...
  // migration 0033 with both generation effects, and migration 0035 removed the
  // generic completion from the repository outright by making CONFIDENCE_BATCH
  // managed; Intent's own typed completion is unchanged.
  assert.doesNotMatch(repository, /async complete\(id:string/u);
