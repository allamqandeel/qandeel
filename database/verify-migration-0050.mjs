...
await client.connect();try{
 ...
 await client.query("INSERT INTO public.him_calculation_results(id,user_id,metric_key,definition_version,model_id,model_version,context_kind,context_id,result_state,numeric_value,missing_input_keys,contradiction_state,supporting_evidence_refs,contradictory_evidence_refs,provenance,confidence_state,confidence_reference,trace_id,update_reason,measurement_event_id,measurement_observation_id,canonical_binding_id,scale_contract_reference,scale_version) VALUES($1,$2,'hse.energy',1,'hse.energy.direct-structured-user-report',1,'CONVERSATION_SESSION',$3,'ASSESSED',2,ARRAY[]::text[],'NONE',ARRAY['measurement-observation:'||$4::text],ARRAY[]::text[],'QANDEEL_HIM_CALCULATION_RUNTIME_V1','UNASSESSED',NULL,$5,'DIRECT_STRUCTURED_USER_REPORT',$6,$4,$7,'hse.energy.ordinal-5.v1',1)",[lateResult,one,session,obs2.id,randomUUID(),obs2.measurement_event_id,v1.id]);
