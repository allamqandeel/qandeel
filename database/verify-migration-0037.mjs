...
await client.connect();try{
 ...
 await client.query('RESET ROLE');
 const oldBindingRead=await client.query("SELECT * FROM public.him_canonical_model_bindings WHERE metric_key='hse.stress' AND definition_version=1 AND context_kind='CONVERSATION_SESSION' AND status='ACTIVE'");if(oldBindingRead.rowCount!==1)throw new Error('Expected exactly the one ACTIVE canonical v1 Stress CONVERSATION_SESSION binding');const oldBinding=oldBindingRead.rows[0],candidate='f8370000-0000-4000-8000-000000000001';
