...
await db.connect();try{
 const constraint=(await db.query("SELECT pg_get_constraintdef(oid) definition FROM pg_constraint WHERE conrelid='public.post_response_intelligence_effects'::regclass AND conname='post_response_intelligence_effects_effect_key_check' AND contype='c'")).rows;
 if(constraint.length!==1)throw new Error('Canonical effect-key constraint missing or duplicated.');const accepted=[...constraint[0].definition.matchAll(/'([A-Z_]+)'/gu)].map(match=>match[1]);if(accepted.length!==canonical.length||canonical.some((key,index)=>accepted[index]!==key))throw new Error(`Effect-key constraint mismatch: ${accepted.join(',')}`);
 const tableAcl=(await db.query("SELECT has_table_privilege('authenticated','public.post_response_intelligence_effects','SELECT,INSERT,UPDATE,DELETE') authenticated,has_table_privilege('anon','public.post_response_intelligence_effects','SELECT,INSERT,UPDATE,DELETE') anon,has_table_privilege('public','public.post_response_intelligence_effects','SELECT,INSERT,UPDATE,DELETE') public")).rows[0];if(tableAcl.authenticated||tableAcl.anon||tableAcl.public)throw new Error('Effect table ACL mismatch.');
 ...
 for(const key of managed)await rejected('SELECT public.claim_post_response_intelligence_effect_v1($1,$2)',[execution,key]);
