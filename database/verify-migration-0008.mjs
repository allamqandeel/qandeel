...
async function main(){ await client.connect(); try{
  ...
  await client.query('BEGIN'); try{
    const user=randomUUID(),other=randomUUID(),hypothesis=randomUUID(),otherHypothesis=randomUUID(),support=randomUUID(),contradict=randomUUID(),otherMemory=randomUUID(),ineligible=randomUUID(); await client.query('RESET ROLE');
    ...
    await client.query(`INSERT INTO public.memories(id,user_id,scope,type,content,source,confidence,importance,status) VALUES
      ...
      ($3,$6,'USER','PERSONAL_FACT','other','USER_STATED',.9,.5,'ACTIVE'),($4,$5,'USER','PERSONAL_FACT','inactive','USER_STATED',.9,.5,'SUPERSEDED')`,[support,contradict,otherMemory,ineligible,user,other]);
