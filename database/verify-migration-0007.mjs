...
async function main(){ await client.connect(); try { await verifyCanonicalContract(); await client.query('BEGIN'); try {
 ...
 const candidate=(await client.query('SELECT * FROM public.create_validated_question_candidate($1::jsonb)',[{id:randomUUID(),informationGapId:gapId,questionText:'  What happened first? ',questionType:'DISCRIMINATING',targetHypothesisIds:[hypothesis],informationNeeded:'order',answerFormat:'FREE_TEXT'}])).rows[0];
 ...
 const beforeCredentialAttempt=Number((await client.query('SELECT count(*) count FROM public.question_candidates WHERE information_gap_id=$1',[gapId])).rows[0].count);
 await rejectsQuery('SELECT * FROM public.create_validated_question_candidate($1::jsonb)',[{id:randomUUID(),informationGapId:gapId,questionText:'What is your password?',questionType:'FACT_FINDING',targetHypothesisIds:[hypothesis],informationNeeded:'user password',answerFormat:'FREE_TEXT'}]);
 const afterCredentialAttempt=Number((await client.query('SELECT count(*) count FROM public.question_candidates WHERE information_gap_id=$1',[gapId])).rows[0].count); assert.equal(afterCredentialAttempt,beforeCredentialAttempt,'credential rejection must not persist a candidate');
...
main().then(()=>console.log('Verified migration 0007 ownership, RLS, canonical metadata, links, uncalibrated fields, duplicate rejection, and direct-RPC credential rejection without persistence.')).catch(error=>{console.error(`Question database verification failed (${error?.code??'verification'}). Connection details were suppressed.`);process.exitCode=1;});
