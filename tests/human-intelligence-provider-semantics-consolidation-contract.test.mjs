...
function assertProviderSemanticsConsolidationContract(sources) {
  ...
  // 13. This task changes NO database file and adds NO migration. Forward-safe
  //     (QHIA-015 phase closure repair): the terminal Human Intelligence
  //     Activation phase migration 0061 must EXIST, and no migration may claim
  //     this task's own provider-semantics consolidation identity - a later,
  //     separately reviewed phase migration is legal by number.
  ...
  if (migrations.some((name) => /provider.*semantic|semantic.*provider|semantics_consolidation/iu.test(name)))
    violated('QHIA-013 adds no migration: no migration claims the provider-semantics consolidation identity');
}
...
test('C5 - QHIA-013 changes no database file', () => {
  // Absolute: the expected database diff for this task is ZERO. Forward-safe
  // (QHIA-015 phase closure repair): the terminal Human Intelligence Activation
  // phase migration 0061 exists and no migration claims this task's own
  // provider-semantics identity; the live repository's future migration
  // numbering is deliberately NOT frozen here.
  const migrations = readdirSync(new URL('database/migrations/', root)).filter((name) => name.endsWith('.sql'));
  ...
  assert.ok(!migrations.some((name) => /provider.*semantic|semantic.*provider|semantics_consolidation/iu.test(name)), 'no migration claims the QHIA-013 provider-semantics consolidation identity');
