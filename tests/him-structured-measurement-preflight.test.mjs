...
test('current-read routing covers exactly every calibrated structured context',()=>{const repository=read('apps/api/src/human-model/him.repository.ts');for(const contract of CALIBRATED_STRUCTURED_METRICS)assert.match(repository,contract.routing);assert.doesNotMatch(repository,/key==='hgs\./);});
