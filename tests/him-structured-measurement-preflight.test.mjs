...
test('current-read routing covers exactly every calibrated structured context',()=>{const repository=read('apps/api/src/human-model/him.repository.ts');for(const contract of CALIBRATED_STRUCTURED_METRICS)assert.match(repository,contract.routing);
 // All 17 canonical calibrated metrics are routed through the manifest's
 // exact approved contexts and nothing else: every routing predicate in
 // the repository must belong to a manifest entry, so no unapproved
 // metric or broadened context can enter the current read.
 const routed=[...repository.matchAll(/key==='([a-z.-]+)'/g)].map(x=>x[1]);
 assert.deepEqual([...new Set(routed)].sort(),[...CALIBRATED_STRUCTURED_METRICS.map(x=>x.metricKey)].sort());});
