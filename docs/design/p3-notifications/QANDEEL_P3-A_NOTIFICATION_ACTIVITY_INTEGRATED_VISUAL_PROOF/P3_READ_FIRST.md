...
> **Status: `P3-A REFINEMENT — READY FOR PRODUCT OWNER + INDEPENDENT REVIEW`.**
...
**Refinement:** on the same branch, from the independently reviewed head `579648f`.
...
Introductions mark**; a bounded copy cleanup.

## What to look at, in this order

1. `boards/18-decision-summary.png` — the whole direction in one view, the refinement decisions, and the only open
   (craft) questions.
2. `boards/06-attention-strip.png` — the strip, and during a Live Call: ordinary attention waits; the call-safe strip
   for the two exceptions, measured against G3's own Analysis.
3. `boards/05-global-activity-entry.png` — the accepted Open Ledger, the Analysis without the entry, and the
   Introductions mark variants.
...
5. `boards/07`, `08` settings (Reduce, «إظهار النوع», the raised Introductions ceiling) · `14` foreground / background
   (31 scenarios) · `17` accessibility and Reduced Motion.
6. `boards/01`–`04`, `09`, `11`–`13`, `15`, `16` — Activity, filters, attention states, permission, indicators, Quiet
   Hours, ceilings, 320 / 390 / 430.
7. `motion/`: `M08` / `M08r` the call-safe strip in the Analysis (and Reduced Motion), `M09` the same at 320 × 568,
...
|---|---|
| `?state=` | `conv` · `conv-strip-shared` · `shared-strip-qandeel` · `conv-strip-system` · `shared-inplace` · `conv-call` · `conv-call-security` · `conv-call-reminder` · `analysis` · `analysis-strip-shared` · `analysis-call` · `analysis-call-security` · `analysis-call-reminder` · `activity` · `activity-stale` · `settings-root` · `notif` · `notif-lock` · `edu` · `edu-boundary` · `edu-notnow` · `shared` |
| `?lang=en` · `?appearance=light` / `system` · `?rm=1` · `?contrast=more` · `?w=320&h=568` / `?w=430&h=932` | device stand-ins |
| `?os=denied` · `?intro=0` · `?filter=…` · `?proactive=reduce` · `?lockintro=L3` · `?g32=CALL_PINNED` | Product conditions |
| `?entry=bell` · `?introglyph=door` | comparison glyphs (Quiet Bell is history only; At the Door is the Introductions comparison) |

**Try:** in `conv`, press an arrival (`sharedReply`) → the strip; press it → «رحلة الصيف». In `analysis-call`, press
...
| `boards/` · `motion/` · `captures/` | 18 boards · 13 clips · 20 key captures |
