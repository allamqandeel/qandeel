...
```text
node source/tools/p3vendor.mjs      # only to re-copy the P2 material from the merged P2-A package (records PROVENANCE.json)
node source/tools/p3pipeline.mjs    # build → 89 captures + 9 phone clips → 18 boards + M06 → COPY_TABLE.md → checks
...
| copy | `tools/p3copy.mjs` | `data/COPY_TABLE.md` |
