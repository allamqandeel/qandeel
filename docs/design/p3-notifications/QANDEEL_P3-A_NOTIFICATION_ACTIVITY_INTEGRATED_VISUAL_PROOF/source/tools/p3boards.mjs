...
import { P3G, ledgerCutVisible, ringCutVisible } from '../src/p3glyphs.mjs';
...
const FOOT = 'QANDEEL · P3-A Notification + Activity — integrated visual proof / decision gate · P3 NOT CLOSED / NOT FROZEN · no production runtime · phones are Product captures at 2× (no harness); every diagnostic is outside them';
...
const construct = (name, label) => `<figure style="width:210px"><div style="position:relative;width:192px;height:192px;background:#131313;border-radius:14px">${grid}<svg width="192" height="192" viewBox="0 0 24 24" style="position:absolute;inset:0;color:#d8d5ca">${P3G[name](48)}</svg></div><figcaption>${label}</figcaption></figure>`;
board('B05', '05-global-activity-entry.png', 'Board 05 — The global Activity entry in the real shell',
  'Does the Activity entry sit in the global upper chrome beside the frozen shell without replacing the three-World navigation, stay neutral in every state (never Brass), and show attention as presence — distinguishable from selection, pressed and focus?',
  ...
  <div class="row">${phone('conv-ad', 'Arabic, Dark: the entry at the START edge of the upper chrome (right); «تحليل المحادثة» and Replay keep their END-side places; the three Worlds keep the navigation.', 0.78)}<div class="card" style="width:1400px"><p class="h3">The one new signature glyph — two bounded variants (task §20), on P2’s N1 construction</p><div class="row" style="margin-top:6px">${construct('ledger', '<b>Open Ledger — RECOMMENDED.</b> Square keyline 16 u, radius 2 u, two equal rows; the QANDEEL cut carried to the lower END corner on the 45° diagonal, visible gap ' + ledgerCutVisible(22) + ' u (N1 ring cut chord: ' + ringCutVisible(22) + ' u). Not a ring: in P2 the ring means a World.')}${construct('bell', '<b>Quiet Bell — comparison.</b> Familiar, but it names ringing; Activity is a quiet record, not an alarm.')}${construct('introMark', '<b>Introductions source mark</b> (rows only). navShared’s two points, the ring not yet whole: two arcs open on the axis between them.')}
  <div class="col">${crop('bell-ad', band('bell-ad', 47, 95), 1.0, 'Bell in the shell, Arabic, Dark')}${crop('bell-el', band('bell-el', 47, 95), 1.0, 'Bell in the shell, English, Light')}${crop('bell-rest-ad', band('bell-rest-ad', 47, 95), 1.0, 'Bell, no attention')}</div></div></div></div>`);

// 06 — the strip
board('B06', '06-attention-strip.png', 'Board 06 — The in-app Attention Strip: when it appears, and when it does not',
  'Does the strip appear only when QANDEEL already has the user’s attention outside the event’s originating context — attached to the upper chrome, short, dismissible, Direct-Entry capable — and stay absent in the same context and during an active Live Call?',
  `<div class="row">${phone('strip-shared-ad', '<b>Shared</b> — in the Personal Conversation, a reply arrives in «رحلة الصيف». Product voice.', 0.7)}${phone('strip-qandeel-ad', '<b>From QANDEEL</b> — inside a Shared World, QANDEEL’s own follow-up (QANDEEL voice, D18).', 0.7)}${phone('strip-system-ad', '<b>System</b> — a new sign-in. MSA, neutral, no red: importance is not a colour.', 0.7)}${phone('inplace-ad', '<b>Same context — no strip.</b> Already in «رحلة الصيف»: the reply simply lands in the thread; no strip, no Push (D51).', 0.7)}${phone('call-ad', '<b>Live Call — no strip.</b> The same reply (and an introduction) are deferred: only the entry’s mark changes; the call line is untouched (G3 §D).', 0.7)}${phone('call-ended-ad', '<b>After the call ends</b> — re-evaluated, the first still-eligible event is presented. Nothing is replayed in bulk.', 0.7)}</div>
...
board('B07', '07-settings-arabic.png', 'Board 07 — General Settings → «الإشعارات والنشاط», Arabic',
  ...
  `<div class="row">${phone('settings-root-ad', 'General Settings (P1 §8: one destination; groups as P1 placed them). The group this proof realizes.', 0.8)}${phone('notif-ad', 'The whole page (tall capture of one scroll).', 0.6)}${phone('notif-lock-ad', 'Lock Screen preview ceiling for Introductions: default «خاصة جدًا».', 0.8)}${phone('notif-denied-ad', 'OS permission denied: said once, plainly, with the device handoff; Activity keeps working.', 0.8)}${phone('notif-noentry-ad', 'Introductions not entered: its section does not exist yet (D26).', 0.6)}</div>
  <p class="note"><b>«قنديل يبادر معايا» controls interruption only</b> — its line says memory, understanding and analysis are unaffected (D35). <b>Security</b> is a statement, not a switch: it always reaches you, even in Quiet Hours, and cannot override the device (D36). <b>Lock Screen</b> rows are ceilings in human words — «خاصة جدًا» / «تنبيه عام» / «إظهار السياق» / «إظهار المعاينة» — never L-codes; none defaults to «إظهار المعاينة». <b>«إعدادات إشعارات الجهاز»</b> hands over to the platform: sound, alert style, channel importance and the OS Lock Screen stay the device’s.</p>`);
board('B08', '08-settings-english.png', 'Board 08 — General Settings → Notifications & Activity, English',
  ...
  `<div class="row">${phone('settings-root-el', 'General Settings.', 0.8)}${phone('notif-el', 'The whole page (tall capture).', 0.6)}${phone('notif-al', 'Arabic, Light (for comparison of appearance).', 0.6)}${phone('notif-lock-el', 'Lock Screen ceiling, Introductions.', 0.8)}${phone('notif-denied-el', 'OS permission denied.', 0.8)}</div>`);
...
const LOCK_ROWS = [['qandeel', FX.EV.proactive, 'Personal / QANDEEL (proactive)'], ['shared', FX.EV.sharedReply, 'Shared World'], ['public', FX.EV.publicReply, 'Public World'], ['intro', FX.EV.introProposal, 'Introductions'], ['security', FX.EV.security, 'System / Account — critical security']];
...
board('B10', '10-lock-screen-disclosure.png', 'Board 10 — Lock Screen disclosure L0 → L3 (platform schematic, not OS UI)',
  'For each category, what may appear outside the authenticated Product at each ceiling — with Introductions defaulting to L0 (not even revealing it is an introduction), L3 never a default, and a critical event still only L2?',
  `<table class="lk"><tr><th style="width:190px">category · D15 default</th>${M.LEVELS.map((l) => `<th>${l} — «${COPY.ar.levels[l].text}» / ${COPY.en.levels[l].text}${l === 'L3' ? ' <span class="tag warn">NEVER A DEFAULT</span>' : ''}</th>`).join('')}</tr>
  ${LOCK_ROWS.map(([subj, ev, name]) => `<tr><td><b style="color:#ece9df">${name}</b><span class="m">default ${M.DISCLOSURE_DEFAULTS[subj]}${subj === 'intro' ? ' — must not reveal Introductions' : subj === 'security' ? ' — critical does not raise it' : ''}</span></td>${M.LEVELS.map((l) => { const allowed = M.LEVELS.indexOf(l) <= M.LEVELS.indexOf(ev.safeMax || 'L3'); const def = M.DISCLOSURE_DEFAULTS[subj] === l; return `<td class="cell"><div class="wall">${ios(pj(ev, allowed ? l : ev.safeMax, 'ar'), 'ar', !allowed)}</div>${def ? '<span class="tag def">DEFAULT</span>' : ''}${!allowed ? `<span class="m">content cap ${ev.safeMax}: QANDEEL renders ${ev.safeMax} even if the user allows more (D17)</span>` : ''}</td>`; }).join('')}</tr>`).join('')}</table>
  <div class="row"><div class="col"><p class="h3">The defaults, English, Android-like schematic</p><div class="wrap" style="margin-top:0">${LOCK_ROWS.map(([subj, ev]) => `<div class="wall">${andr(pj(ev, M.DISCLOSURE_DEFAULTS[subj], 'en'), 'en')}</div>`).join('')}</div></div></div>
  <p class="note"><b>QANDEEL owns the words; the platform owns the frame.</b> The cards are schematic stand-ins for the iOS / Android presentation, not reproductions, and the app icon is a grey placeholder. The platform can still hide more: iOS “Show Previews” (Always / When Unlocked / Never, user-owned) and Android’s per-channel Lock Screen visibility (the user “always has ultimate control”). QANDEEL’s level is a ceiling the renderer may undershoot — never exceed (D17). Every string above is produced by <code>model.project()</code> from ONE event, so no preview combines two Worlds.</p>`, lockCss);
...
board('B14', '14-foreground-background.png', 'Board 14 — Foreground and background: no duplicate interruption',
  'When QANDEEL already has the user’s attention, is an external Push avoided — the event lands in place in the same context, as a strip in a different context, deferred during a Live Call — while in the background Push is used only when eligible and permitted, and Activity is always the fallback?',
  `<div class="row"><table style="width:1290px"><tr><th>scenario</th><th>where the user is</th><th>event</th><th>model</th><th>expected</th><th>reasons</th></tr>${scen.map(({ sc, r }) => `<tr><td>${sc.id} ${sc.name}</td><td>${sc.app}${sc.here ? ' · ' + sc.here : ''}${sc.liveCall ? ' · LIVE CALL' : ''}${sc.os ? ' · OS ' + sc.os : ''}</td><td>${sc.ev}</td><td><b style="color:#ece9df">${r.surface}</b></td><td class="${r.surface === sc.expect ? 'ok' : 'no'}">${sc.expect}</td><td>${r.reasons.join(', ')}</td></tr>`).join('')}</table>
  <div class="col">${phone('inplace-ad', 'S1 same context: in place', 0.5)}${phone('strip-shared-ad', 'S2 different context: strip', 0.5)}</div><div class="col">${phone('call-ad', 'S7/S8 Live Call: deferred, mark only', 0.5)}${phone('notif-denied-ad', 'S6 OS denied: Push unavailable; Activity stays', 0.5)}</div></div>
...
async function a11yTable() {
  const rows = [];
  for (const [st, lang] of [['conv-strip-shared', 'ar'], ['activity', 'ar'], ['notif', 'en'], ['edu', 'ar']]) {
...
function stripFrames(id) {
  const t = truth(id), picks = [15, 16, 18, 23, 60, 195, 197, 200];
  const r = SH['strip-shared-ad'].rects['.strip'][0];
...
const DEC = [
  ...
  ['Entry', 'the new Open Ledger glyph (recommended; Quiet Bell compared) at the START of the upper chrome on every non-Analysis world surface; neutral rest ink in every state'],
  ['Attention', 'a neutral presence mark (never Brass, never red, never a number); inside Activity a hollow WAITING mark for items that still need you; counts only for Shared and actionable System'],
  ['In-app attention', 'the Attention Strip under the chrome — only outside the originating context, never in a Live Call; deferred events re-evaluated after the call'],
  ['Outside the app', 'the platform’s own notification presentation; QANDEEL decides only the words, by the L0–L3 ceiling; Introductions L0; L3 never default'],
  ['Settings', 'General Settings → «الإشعارات والنشاط»: Proactive (Allow / Reduce / Off), Shared + per-World mute, Public (+ opt-in Discovery), Introductions (once entered), Security (always), Quiet Hours 23:00 → 08:00, Snooze 1 h / 8 h / 24 h / Custom, Lock Screen previews, device handoff'],
...
const OPEN = [
  'Entry glyph: Open Ledger (recommended) or Quiet Bell — the one new signature drawing.',
  'Introductions source mark in Activity rows (two arcs + two points) — a second semantic exception to “one opening per ring”.',
  'The L1 human label «تنبيه عام» / “General” — no directional word was given for L1.',
  'What “Reduce / أقل” means precisely — the proof reads it as “Timely reasons only”.',
  'Critical security during an active Live Call — the proof defers its strip to the call’s end (the mark still shows at once).',
  'The Activity entry is not added to the Analysis chrome (G3 composition untouched) — reachable after «المحادثة».',
  'Final Arabic / English copy of every proof string (content.mjs statuses), and the QANDEEL-voice register per user dialect.',
];
...
async function main(only) {
  ...
  board('B18', '18-decision-summary.png', 'Board 18 — Decision summary for the Product Owner',
    'The integrated direction in one view: what the user sees, where, and when — realizing the accepted P3 direction without reopening I-08N-01, P1 or P2 — and the only questions left, which are craft.',
    `<div class="row">${phone('conv-ad', 'The entry, attention present', 0.52)}${phone('act-ad', 'Activity', 0.52)}${phone('strip-shared-ad', 'The strip', 0.52)}${phone('call-ad', 'Live Call: nothing interrupts', 0.52)}${phone('edu-ad', 'Permission, in context', 0.52)}${phone('notif-lock-ad', 'Lock Screen ceiling', 0.52)}</div>
    <div class="row"><div class="card" style="width:1100px"><p class="h3">Realized — the accepted P3 direction</p><table>${DEC.map(([a, b]) => `<tr><td style="width:180px"><b style="color:#ece9df">${a}</b></td><td>${b}</td></tr>`).join('')}</table></div>
    <div class="card" style="width:660px"><p class="h3">Open craft questions only (nothing already accepted is re-asked)</p><ol style="margin:0;padding-inline-start:18px;font-size:13.5px;line-height:1.6">${OPEN.map((o) => `<li>${o}</li>`).join('')}</ol></div></div>`);
