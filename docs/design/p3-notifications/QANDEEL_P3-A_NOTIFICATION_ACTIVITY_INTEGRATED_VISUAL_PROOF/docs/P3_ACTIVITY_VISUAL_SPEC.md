...
**Status:** `P3-A PROOF SPECIFICATION — NOT FROZEN`. Numbers marked *craft* are reviewed reference values, not
...
- **Not in the Analysis chrome.** G3 froze that composition and the Analysis must not be redesigned for notifications
  (task §19). From the Analysis, Activity is one step away through «المحادثة». *(open craft question 6)*
...
| | |
|---|---|
| Recommended | **Open Ledger** — square keyline 16 u, corner radius 2 u, two equal rows; the QANDEEL cut carried to the lower END corner on the 45° diagonal, visible gap 5.69 u at 22 px (N1's ring cut: 6.76 u chord) |
| Compared | **Quiet Bell** — familiar, but it names ringing |
...
- **Row grammar:** 28-pt source glyph column (the P2 World glyphs in neutral secondary ink; the Introductions source
  mark; Hugeicons Free `settings` for System) · line 1 (12 / 500): source name, time, and when needed a state word
