...
| **More than two readings for one subject** — the case `p3` exercises | The runtime caps active `hypotheses` at 32 per user (`MAX_ACTIVE_HYPOTHESES`), and no per-subject cap was found, so one subject can carry more than two readings. V9 assumed exactly two; that assumption was not backed. The runtime is bounded, not unlimited, and nothing here claims all 32 belong to one subject |
