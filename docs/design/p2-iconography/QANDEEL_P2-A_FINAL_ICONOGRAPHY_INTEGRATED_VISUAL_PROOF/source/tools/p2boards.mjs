...
import { RAIL_VARIANTS, SPINE_VARIANTS, RAIL_RECOMMENDED, SPINE_RECOMMENDED, railArt, END_GLYPH_PX } from '../src/machines.mjs';
...
const STATUS = 'QANDEEL · P2-A Final Iconography — integrated visual proof / decision gate · PRODUCT OWNER VISUAL SELECTIONS ACCEPTED · P2 NOT CLOSED / NOT FROZEN (canonical closure is the later P2-B task) · phones are Product captures at 2× (no harness); every diagnostic is outside them';
...
<p class="k" style="margin-top:26px">N1 “OPEN” — ACCEPTED BY THE PRODUCT OWNER · 48 px · dark World #101010 · Brass ${DK.brass} / neutral ${DK.primary}</p>${glyphRow('open', 48, DK.world, DK.primary, DK.brass)}
...
<p class="k" style="margin-top:22px">N2 “ARCHITECTURAL” — COMPARISON EVIDENCE ONLY, NOT SELECTED (flat terminals at the cut, 34° cut, +8 % stroke, tighter radii) · 48 px and 24 px</p>
...
board('B02', '02-utility-library-comparison.png', () => page('02', 'Utility family — the same eight meanings in five libraries, beside the QANDEEL signature glyphs',
  ...
  LIBS.map((l) => { const L = UTIL.libraries[l]; return `<tr><td style="width:210px"><b style="color:#ecebe4">${esc(L.title)}</b>${l === UTILITY_DEFAULT ? '<span class="tag">ACCEPTED SOURCE</span>' : ''}<br><span style="color:#8b8982;font:11px ui-monospace,Consolas,monospace">${L.pkg}@${L.version}<br>${L.license}${l === 'phosphor' ? ' · RN wrapper is community-owned' : ''}</span></td>
...
// ---------------------------------------------------------------------------------------------------------- B04
board('B04', '04-call-rail-variants.png', () => page('04', `Call Rail — ${RAIL_RECOMMENDED} accepted by the Product Owner; B and C preserved as comparison evidence`,
  'The comparison the selection was made from (not an open choice): which rail best makes Mic + audio route ONE group and End Call a related but separated terminal — without circles, a full pill, red, Brass or a literal Q? Same call, same moment, three machines. Neutral hairline only; End\'s rank = separation + terminal form + its solid glyph in primary ink.',
  ['A', 'B', 'C'].map((v) => `<div class="row" style="gap:22px;align-items:flex-end"><div style="width:250px"><h1 style="font-size:19px">${v} · ${RAIL_VARIANTS[v].name}${v === RAIL_RECOMMENDED ? '<span class="tag">ACCEPTED</span>' : '<span class="tag" style="background:#3a3a3a;color:#bbb">EVIDENCE ONLY</span>'}</h1><p class="note">${esc(RAIL_VARIANTS[v].note)}</p></div>${railCrop(`rail${v}-call-live-ar`, 1.35, 'Analysis · FOLLOW_LIVE · Arabic')}${railCrop(`rail${v}-conv-call-ar`, 1.35, 'Conversation · same call')}</div>`).join('') +
...
// ---------------------------------------------------------------------------------------------------------- B05
board('B05', '05-call-rail-recommended.png', () => page('05', `Call Rail ${RAIL_RECOMMENDED} “${RAIL_VARIANTS[RAIL_RECOMMENDED].name}” (accepted) in the real Product — End Call at ${END_GLYPH_PX} px`,
...
<div class="row">${railCrop('rec-call-live', 1.6, 'Mic open · speaker route')}${railCrop('rec-muted', 1.6, 'Mic muted · earpiece route — form changes, not colour')}</div>`,
  `Mute is carried by FORM (the slash cuts a band of negative space through the microphone), the speaker route by fill + wave count; End Call is the only solid mark and sits beyond the seam at the END edge; since the refinement its handset renders at ${END_GLYPH_PX} px (was 24; board 16), with the plate, the seam and every target unchanged. The call line keeps G3.2's 64 pt height, so the world floor and the temporal row are untouched (K14 geometry unchanged).`));

// ---------------------------------------------------------------------------------------------------------- B06
board('B06', '06-temporal-spine-variants.png', () => page('06', `Temporal Spine + Aperture — ${SPINE_RECOMMENDED} accepted by the Product Owner; A and B preserved as comparison evidence`,
  'The comparison the selection was made from (not an open choice): which aperture opens time most clearly — committed vs preview distinguishable, never a generic circular thumb, and a Live terminal that can never be read as a Moment? Rows: variant. Columns: FOLLOW_LIVE · PINNED(14) · PINNED(17 = Moment(LH)) · preview of 16 under the finger (committed 14 stays).',
  ['A', 'B', 'C'].map((v) => `<div class="row" style="gap:14px;align-items:flex-start"><div style="width:190px"><h1 style="font-size:19px">${v} · ${SPINE_VARIANTS[v].name}${v === SPINE_RECOMMENDED ? '<span class="tag">ACCEPTED</span>' : '<span class="tag" style="background:#3a3a3a;color:#bbb">EVIDENCE ONLY</span>'}</h1><p class="note">${esc(SPINE_VARIANTS[v].note)}</p></div>` +
...
// ---------------------------------------------------------------------------------------------------------- B07
board('B07', '07-temporal-spine-recommended.png', () => page('07', `Temporal Spine ${SPINE_RECOMMENDED} “${SPINE_VARIANTS[SPINE_RECOMMENDED].name}” (accepted) through every temporal state`,
...
${stateCell('speaker (pressed=true)', sigSvg('routeOn'))}${stateCell('earpiece (false)', sigSvg('route'))}${stateCell(`End · REST · ${END_GLYPH_PX} px`, sigSvg('endCall', { size: END_GLYPH_PX }), { ink: DK.primary })}${stateCell('End · PRESSED', sigSvg('endCall', { size: END_GLYPH_PX }), { ink: DK.primary, wash: true })}${stateCell('End · FOCUS', sigSvg('endCall', { size: END_GLYPH_PX }), { ink: DK.primary, ring: true })}</div>
...
// ---------------------------------------------------------------------------------------------------------- B16
// P2-A refinement: End Call glyph presence inside the ACCEPTED Call Rail A (data/END_CALL_STUDY.json).
board('B16', '16-end-call-presence-26-27-28.png', () => {
  const ST = JSON.parse(readFileSync(join(PKG, 'data', 'END_CALL_STUDY.json'), 'utf8'));
  const endZoom = (id, z, cap) => { const b = SHOTS[id].measured.boxes['end-call']; return crop(id, b.x - 8, b.y - 4, b.w + 16, b.h + 8, z, cap, id); };
  const col = (px) => `<div style="width:420px"><h1 style="font-size:19px;margin-top:6px">${px} px${px === 24 ? ' — the reviewed proof' : ''}${px === ST.selected ? '<span class="tag">SELECTED</span>' : ''}</h1>
${railCrop(`end${px}-live-ar`, 1.0, 'Analysis · Arabic')}${railCrop(`end${px}-live-en`, 1.0, 'Analysis · English')}${railCrop(`end${px}-light-ar`, 1.0, 'Conversation · system Light')}
<div class="row" style="gap:10px;margin-top:10px">${endZoom(`end${px}-live-ar`, 2.4, 'REST')}${endZoom(`end${px}-focus-ar`, 2.4, 'FOCUS')}${endZoom(`end${px}-press-ar`, 2.4, 'PRESSED')}</div></div>`;
  const rows = ST.studySizes.map((px) => { const s = ST.summary[px], v = ST.verdict[px], L = ST.bySize[px].shots['light-ar'];
    return `<tr><td><b>${px} px</b>${px === ST.selected ? '<span class="tag">SELECTED</span>' : ''}</td><td>${s.inkAr.w} × ${s.inkAr.h}</td><td>${s.endWeightAr}</td><td>${(s.endWeightAr / ST.summary[24].endWeightAr).toFixed(2)} ×</td><td>${L.end.weight} vs Route ${L.route.weight}</td><td>${s.minClearance}</td>${Object.keys(ST.criteria).map((k) => `<td class="${v[k].pass ? 'pass' : ''}">${v[k].pass ? 'pass' : 'FAIL'}</td>`).join('')}</tr>`; }).join('');
  return page('16', `End Call glyph presence — 24 / 26 / 27 / 28 px inside the accepted Call Rail A`,
    'Product Owner refinement: with End Call kept neutral (no red, no Brass), can its handset carry slightly more hierarchy through optical size alone, while the plate, the seam, the 44-pt targets and the Mic / Route glyphs stay exactly as accepted? Same drawing, scaled; measured in the real Product.',
    `<div class="row" style="gap:22px">${ST.studySizes.map(col).join('')}</div>
<p class="k" style="margin-top:22px">320 × 568 — IN A CALL, PINNED · the tightest reviewed width</p><div class="row" style="gap:22px;margin-top:4px">${ST.studySizes.map((px) => railCrop(`end${px}-s320-ar`, 1.25, `${px} px`)).join('')}</div>
<h1 style="font-size:17px;margin-top:26px">Measured (data/END_CALL_STUDY.json · tools/p2endstudy.mjs) — contrast-weighted ink in pt², Arabic Analysis unless stated</h1>
<table><tr><th>size</th><th>ink box pt</th><th>End weight</th><th>vs 24 px</th><th>Light: End vs Route</th><th>clearance to target pt</th>${Object.keys(ST.criteria).map((k) => `<th>${k}</th>`).join('')}</tr>${rows}</table>
<table style="margin-top:14px">${Object.entries(ST.criteria).map(([k, t]) => `<tr><td style="width:40px">${k}</td><td>${esc(t)}</td></tr>`).join('')}</table>
<p class="q" style="margin-top:14px"><b>Selected: ${ST.selected} px.</b> ${esc(ST.reason)}. Mic ${ST.summary[24].micWeightAr} and Route ${ST.summary[24].routeWeightAr} pt² are unchanged at every size.</p>`,
    (() => { const lead = (px) => { const L = ST.bySize[px].shots['light-ar']; return Math.round((L.end.weight / L.route.weight - 1) * 100); };
      return `What the numbers say: at 24 px, under system Light, End Call is ${lead(24) < 0 ? 'LIGHTER than' : 'barely heavier than'} the speaker Route beside it (${lead(24)} %: the review's concern, measured). Over the Light Route, 26 px leads by ${lead(26)} %, 27 px by ${lead(27)} %, 28 px by ${lead(28)} % with a ${ST.summary[28].inkAr.w}-pt-wide handset. All three stay inside the target with ≥ ${Math.min(...[26, 27, 28].map((p) => ST.summary[p].minClearance))} pt clear, below the whole group's weight, identical in Arabic and English, and unchanged under focus and press. None fails, so the task's default, 27 px, stands. At 320 × 568 the target does not move (H1) and the ink stays inside it (H2), so no new collision is possible.`; })());
});
