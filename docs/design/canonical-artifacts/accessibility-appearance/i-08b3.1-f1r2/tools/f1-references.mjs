/**
 * I-08B3.1-F1 — THE REFERENCE GATE.
 *
 * Every row records: the EXACT source, WHAT IT SAYS in its own words, WHAT IT CHANGED in F1,
 * WHAT IT DID NOT CHANGE, and WHERE QANDEEL DID SOMETHING DIFFERENT and why. A row that changed
 * nothing is still a row — a reference read and found not to apply is a result, and hiding it
 * would make the gate look more decisive than the research was.
 *
 * REACHABILITY IS RECORDED RATHER THAN PARAPHRASED. reactnative.dev is unreachable from this
 * host — I-08B3.1-E1 recorded the same thing — so the React Native rows below are taken from
 * the SOURCE on the main branch, fetched and quoted, which is a stronger primary than the docs
 * site would have been. That is stated rather than smoothed over.
 */
...
export const REFERENCES = [
  /* ------------------------------------------------------------------------ APPLE ------ */
  {
    source: 'Apple — Human Interface Guidelines, Accessibility (change log: "March 7, 2025 — Expanded and refined all guidance")',
    url: 'https://developer.apple.com/design/human-interface-guidelines/accessibility',
    fetched: 'rendered in a headless browser; the page is a JS application and returns only a shell to a plain fetch',
    says: [
    ...
    changed:
      'THE BLUR CLAUSE IS WHY qandeel.accessibility.motion.blur EXISTS AS A SEPARATE CHANNEL rather than being folded into "travel". It is also the clause I-08B3.1-D2R corrected the inherited D1 Connection for, and F1 inherits that correction rather than re-making it. The x/y/z-to-fades clause is the licence for the GUIDED THREAD\'s reduced counterpart: origin and destination both present, the relation arriving by LEVEL, no path drawn.',
    didNotChange:
      'The CONDITIONAL in the contrast sentence changed the FRAMING of Part B and deserves saying: QANDEEL\'s default already meets 4.5:1 for every reading rung — primary 12.95:1, secondary 8.38:1, tertiary 5.44:1. So Increase Contrast here is NOT a remedy for a failing default. It is an additional legitimate expression, which is exactly what §0 of the brief insists on.',
    qandeelDifferently:
      'Apple says "reducing… scaling". QANDEEL\'s ambient field has a SCALE that is a depth cue and never animates, so there is no scaling to reduce — and rather than claim a compliance it did not earn, the motion inventory records the ambient field\'s stillness as a decision already taken by D2R.',
  },
  {
    source: 'Apple — Human Interface Guidelines, Motion (change log: "September 9, 2025 — Added guidance for Liquid Glass")',
    url: 'https://developer.apple.com/design/human-interface-guidelines/motion',
    fetched: 'rendered in a headless browser',
    says: [
    ...
    changed:
      'The fade clauses are the named alternative the GUIDED THREAD and the CONNECTION counterparts use, so the replacement channel is Apple\'s own rather than invented. "Avoid using motion as the ONLY way to communicate important information" is the sentence that makes qandeel.accessibility.motion a set of CHANNELS rather than a switch.',
    didNotChange:
      'The 0.2 Hz clause changed nothing in F1 and is recorded because of that: the ambient field is still, so there is nothing at that frequency to remove. A system with a breathing ring would have had to delete it here.',
    qandeelDifferently: null,
  },
  {
    source: 'Apple — Human Interface Guidelines, VoiceOver (change log: "March 7, 2025 — New page")',
    ...
    didNotChange:
      'The rotor recommendation did not change the design because it COULD not: React Native does not expose UIAccessibilityCustomRotor. Headings are the nearest mechanism RN can express and are traversable by both VoiceOver and TalkBack, and the difference is recorded as a platform gap rather than hidden.',
    qandeelDifferently: null,
  },
  {
    source: 'Apple — Human Interface Guidelines, Typography (Specifications: iOS/iPadOS Dynamic Type and larger accessibility type sizes)',
    url: 'https://developer.apple.com/design/human-interface-guidelines/typography',
    fetched: 'rendered in a headless browser; the size tables were read out of the rendered DOM, per table, rather than recalled',
    says: [
  ...
  },
  {
    ...
    fetched: 'rendered in a headless browser',
    says: ['"The appearance of these variants can differ in response to certain system settings, like if people choose a preferred look for Liquid Glass in their device\'s settings, or turn on accessibility settings that reduce transparency or increase contrast in the interface."'],
    changed: 'Nothing.',
    didNotChange:
      'AND THE NOTHING IS THE RESULT. Apple\'s Reduce Transparency guidance is about Liquid Glass and the standard blur/vibrancy materials. QANDEEL has none of them: the functional Surface is an opaque matte, the navigation is opaque, CHROME.opacity is 1, and I-08B3.1-B4R recorded "Reduce Transparency has nothing to undo here" before this task existed. A package that shipped a chrome transformation for this setting would have been transforming something that does not exist.',
    qandeelDifferently:
      'So Part C is redirected onto the three subjects QANDEEL actually has — the ambient field, the PASSAGE scrim and the Light — and one of them, the Light, deliberately does not change. That redirection is F1\'s, not Apple\'s.',
  },
  /* -------------------------------------------------------------------------- W3C ------- */
  {
    source: 'W3C — Understanding WCAG 2.2, SC 1.4.1 Use of Color',
    url: 'https://www.w3.org/WAI/WCAG22/Understanding/use-of-color.html',
    fetched: 'fetched directly',
    says: [
    ...
    changed:
      'The 3:1 escape hatch is what made E1 measure the error ink against the reading ramp and find it CLOSED at 1.03:1. F1 inherits that measurement and check N-03 re-runs it, which is why the error role\'s glyph, copy and role are treated as an obligation rather than a courtesy.',
    didNotChange:
      'It did not change the atmosphere palette, and the reason is the more interesting finding: the six hues measure a maximum of 1.0185:1 against EACH OTHER, so colour is not the only carrier there because colour is not a carrier there at all.',
    qandeelDifferently: null,
  },
  {
    source: 'W3C — Understanding WCAG 2.2, SC 1.4.11 Non-text Contrast',
    url: 'https://www.w3.org/WAI/WCAG22/Understanding/non-text-contrast.html',
    fetched: 'fetched directly',
    says: ['A 3:1 threshold for graphical objects required to understand the content, and for the visual indicators of user interface components.'],
    changed:
      'ADOPTED AS THE INCREASED-CONTRAST TARGET, AND THE WORD "ADOPTED" IS DOING WORK. I-08B3.1-D2R argued on the record that a contour is atmosphere, carries nothing position and label do not also carry, and is therefore NOT a graphical object under this SC. F1 does not reopen that. It borrows 3:1 as the TARGET for an expression a user asked for, and tools/f1-derive.mjs says so in its header so that nobody later reads the number as an obligation the default is failing.',
    didNotChange: 'The DEFAULT expression. 66 of 90 contour samples sit below 3:1 in the default and that remains correct under D2R\'s argument.',
    qandeelDifferently: null,
  },
  {
    source: 'W3C — Understanding WCAG 2.2, SC 2.5.7 Dragging Movements',
    url: 'https://www.w3.org/WAI/WCAG22/Understanding/dragging-movements.html',
    fetched: 'fetched directly',
    says: ['A single-pointer alternative is required for author-controlled dragging operations.'],
    changed:
    ...
    didNotChange: 'Nothing that ships in F1 — the Map\'s gesture system is out of scope.',
    qandeelDifferently: null,
  },
  {
    source: 'W3C — Understanding WCAG 2.2, SC 1.4.4 Resize Text and SC 1.4.10 Reflow',
    url: 'https://www.w3.org/WAI/WCAG22/Understanding/resize-text.html',
    fetched: 'fetched directly',
    says: ['Text must be resizable up to 200 % without loss of content or functionality; content must reflow without two-dimensional scrolling.'],
    changed: 'The 200 % row exists in the parity matrix as its own expression rather than being folded into the AX5 row, so the standard\'s own threshold is reported separately from the platform\'s maximum.',
    didNotChange: 'The AX5 test point, which comes from Apple\'s table and is higher.',
    qandeelDifferently: null,
  },
  {
    source: 'W3C — Understanding WCAG 2.2, SC 2.3.3 Animation from Interactions (AAA)',
    url: 'https://www.w3.org/WAI/WCAG22/Understanding/animation-from-interactions.html',
    fetched: 'fetched directly',
    says: ['Motion animation triggered by interaction can be disabled, unless the animation is essential to the functionality or information being conveyed.'],
    changed: 'The word ESSENTIAL is the second licence — alongside the accessibility skill\'s "non-essential" — for keeping semantic motion in a reduced expression instead of deleting it.',
    didNotChange: null,
    qandeelDifferently: null,
  },
  /* ------------------------------------------------------------------ REACT NATIVE ------ */
  {
    source: 'React Native — Libraries/Components/AccessibilityInfo/AccessibilityInfo.js, main branch (SOURCE, not the docs site)',
    url: 'https://raw.githubusercontent.com/facebook/react-native/main/packages/react-native/Libraries/Components/AccessibilityInfo/AccessibilityInfo.js',
    fetched: 'downloaded and read line by line. reactnative.dev is UNREACHABLE from this host — I-08B3.1-E1 recorded the same — so the source was used, which is a stronger primary than the docs site.',
    says: [
    ...
    changed:
      'THE ENTIRE PLATFORM MAPPING, AND IT KILLED A CROSS-PLATFORM STORY. Increase Contrast is NOT one signal: it is isDarkerSystemColorsEnabled on iOS and isHighTextContrastEnabled on Android, which are two different settings with two different scopes — one is a system-wide contrast preference, the other is specifically about TEXT. Reduce Transparency has NO Android equivalent at all. So the combined-settings matrix has fewer real cells on Android than on iOS, and F1_PLATFORM_MAPPING.md says which rather than implying parity.',
    didNotChange: null,
    qandeelDifferently:
      'A third-party binding consulted alongside the source lists isHighTextContrastEnabled and isDarkerSystemColorsEnabled as available on BOTH platforms. The source says otherwise, and the source wins. Recording the disagreement is the point: the plausible secondary is what a package would ship if it did not check.',
  },
  {
    source: 'React Native — ReactAndroid/.../accessibilityinfo/AccessibilityInfoModule.kt, main branch (SOURCE)',
    url: 'https://raw.githubusercontent.com/facebook/react-native/main/packages/react-native/ReactAndroid/src/main/java/com/facebook/react/modules/accessibilityinfo/AccessibilityInfoModule.kt',
  ...
  /* -------------------------------------------------------------- SOFTWARE MANSION ------ */
  {
    source: 'Software Mansion — Reanimated, Accessibility guide',
...
export function gate() {
  return {
    generatedBy: 'tools/f1-references.mjs',
    hostReachability: {
    ...
    changedSomething: REFERENCES.filter((r) => r.changed && r.changed !== 'Nothing.').length,
    changedNothingAndSaysSo: REFERENCES.filter((r) => !r.changed || r.changed === 'Nothing.').length,
    divergencesRecorded: REFERENCES.filter((r) => r.qandeelDifferently).length,
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  const g = gate();
  writeFileSync(join(PKG, 'data/F1_REFERENCES.json'), JSON.stringify(g, null, 2) + '\n');
  console.log('references:', g.count, '| changed something:', g.changedSomething, '| changed nothing and says so:', g.changedNothingAndSaysSo, '| divergences:', g.divergencesRecorded);
  for (const r of g.references) console.log('  ' + r.source);
}
