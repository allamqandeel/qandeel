/**
 * I-08B3.1-F1 — the archive. Carried from I-08B3.1-E1 in mechanism, with one exclusion added.
 *
...
function collect() {
  ...
  const walk = (d) => {
    for (const e of readdirSync(d, { withFileTypes: true }).sort((a, b) => a.name.localeCompare(b.name))) {
      // review/src holds the intermediate captures the boards are composed from. A build
      // artefact, not a deliverable, and excluded from the manifest for the same reason.
      if (e.name === '.build' || e.name === 'src') continue;
