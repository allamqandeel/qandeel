/**
 * I-08B3.1-F2 — the archive. The mechanism is I-08B3.1-F1's, with ONE EXCLUSION REMOVED.
 *
 * A zero-dependency store/deflate ZIP writer. ZIP entry names use FORWARD SLASHES on every
 * platform, including this one, which is the trap that has produced an archive that extracts into
 * a single file with backslashes in its name.
 *
 * ------------------------------------------------------------------------------------------------
 * WHY `review/src` IS IN THIS ARCHIVE WHERE I-08B3.1-F1 EXCLUDED IT.
 *
 * F1 called `review/src` "the intermediate captures the boards are composed from — a build artefact,
 * not a deliverable", and F1's boards EMBEDDED every tile as a base64 data URI, so the boards were
 * whole without it.
 *
 * F2's boards REFERENCE their tiles by path instead. That decision halved the package and made the
 * board HTML readable, and it turns `review/src` from an intermediate into a DEPENDENCY: excluding
 * it would ship twelve board documents whose images are all missing. So it is included, and the
 * archive is larger for it.
 * ------------------------------------------------------------------------------------------------
 */
