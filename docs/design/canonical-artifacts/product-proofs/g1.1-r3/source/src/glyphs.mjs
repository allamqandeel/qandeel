...
/**
 * A static voice-message waveform: deterministic bars (a pure function of the seed), drawn left-to-right in
 * both scripts — a media timeline is not mirrored (Material bidirectionality; designing-arabic-frontends §6).
 */
export function waveSVG(seed, { bars = 30, w = 120, h = 22 } = {}) {
  const r = (n) => { const x = Math.sin((n + seed * 7.13) * 127.1 + 311.7) * 43758.5453; return x - Math.floor(x); };
  const step = w / bars, bw = 2;
  let d = '';
  for (let i = 0; i < bars; i++) {
    const env = Math.sin(Math.PI * (i + 0.5) / bars) ** 0.6;
    const bh = Math.max(3, (0.25 + 0.75 * r(i)) * env * h);
    const x = (i * step + (step - bw) / 2).toFixed(2), y = ((h - bh) / 2).toFixed(2);
    d += `<rect x="${x}" y="${y}" width="${bw}" height="${bh.toFixed(2)}" rx="1"/>`;
  }
  return `<svg class="wave" width="${w}" height="${h}" viewBox="0 0 ${w} ${h}" aria-hidden="true" focusable="false" fill="currentColor">${d}</svg>`;
}
