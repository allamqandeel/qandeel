...
(function () {
  'use strict';
  var D = window.__G31DATA, LANG = window.__G31BOOT.lang;
  ...
  function enterAnalysis() {
    ...
    blend('bgd', 1, DUR.depthOut);                                   // shell candidate A: the shell darkens with it
  ...
  function bandContent() {
    ...
    if (S.ptc != null) lines.push({ cls: 'o-temporal', text: L.previewLine(String(S.ptc)) });
    else if (S.mode === 'PINNED') {
      lines.push({ cls: 'o-temporal', text: L.temporalPinned(String(S.tc)) });
  ...
  function renderBand() {
    var c = bandContent(), key = JSON.stringify(c) + S.ptc + '|' + S.moreOpen + '|' + S.call + '|' + root.clientHeight;
  ...
  function bandRoom() { var f = worldFloor(); return Math.max(54, Math.floor(f.usable - f.floor - 88)); }
  function bandScrollState() { var b = $('band'); var over = b.scrollHeight > b.clientHeight + 1; b.classList.toggle('scroll', over); b.classList.toggle('at-end', over && b.scrollHeight - b.clientHeight - b.scrollTop <= 1); }
  var tlKey = '', tlN = 0;
  ...
  /* Appearance. F2: the Product follows the system. G2 §F: the Living Analysis World is the same dark world either way.
     Q-LIGHT-SHELL (OPEN) — harness fixture only, under system Light:
       A — the elements G2.2 blended (rail, call line, status region, home indicator, Replay) go dark with the Analysis;
       B — the rail, call line, status region and home indicator keep following the system; only what is drawn ON the
           world keeps the world's dark ink (it sits on the world's own material in both candidates). */
  ...
  var MIX_ALL = ['rail', 'composer', 'replay', 'rmenu', 'status', 'homebar'];
  var MIX_ON_WORLD = ['replay', 'rmenu'];
  ...
  function shell() { return root.getAttribute('data-shell') === 'B' ? 'B' : 'A'; }
  function applyAppearance() {
    var ap = appearance(), sh = shell(), t = get('bgd'), key = ap + '|' + sh + '|' + (ap === 'light' ? t.toFixed(4) : '');
    if (key === lastMix) return; lastMix = key;
    var P = D.palettes, mixed = sh === 'A' ? MIX_ALL : MIX_ON_WORLD;
    MIX_ALL.forEach(function (id) {
      var el = $(id); if (!el) return;
      if (ap !== 'light' || mixed.indexOf(id) < 0) { MIXV.forEach(function (v) { el.style.removeProperty(v[0]); }); el.style.removeProperty('--world-rgb'); el.style.removeProperty('--press'); return; }
  ...
  /* The line IS the microphone (G1.1 / G1.2): it draws the reader's level while the microphone carries speech and lies
     flat while QANDEEL speaks or the microphone is muted; QANDEEL's own audio is never drawn. The pen is at the END edge. */
  function hash(n) { var x = Math.sin(n * 127.1 + 311.7) * 43758.5453; return x - Math.floor(x); }
  function simLevel(t) {
    if (t < 280) return 0;
    var u = t - 280, syl = 205, i = Math.floor(u / syl), f = (u % syl) / syl;
    if (i % 7 === 6) return 0.02;
    var amp = 0.35 + 0.6 * hash(i + Math.floor(i / 7) * 13), env = Math.sin(Math.PI * Math.min(1, f * 1.15)); return amp * env * env;
  }
  function drawTrace() {
    var path = $('trace-path');
    var note = S.composer === 'note', live = S.call === 'live';
    if ((!note && !live) || RM) { path.setAttribute('d', 'M0 10 H10'); return; }
    var t = note ? now() - S.noteStart : callT(), n = 48, d = '';
    for (var i = 0; i < n; i++) {
      var tt = t - (n - 1 - i) * 90, lv = 0;
      if (tt >= 0) lv = note ? simLevel(tt) : (speakingAt(tt) === 'me' ? simLevel(tt % 3600) : 0);
  ...
  function truth() {
    ...
    return { state: S.state || null, lang: LANG, place: S.place, rail: S.world, TM: S.mode, TC: S.tc, PTC: S.ptc, LH: S.LH, disclosedMoments: disclosedCount(),
      ...
      appearance: appearance(), shell: shell(), analysisShellDark: +get('bgd').toFixed(3), reducedMotion: RM,
  ...
  window.__G31 = {
    ...
    act: function (name, arg) {
      var fns = { enterAnalysis: enterAnalysis, leaveAnalysis: leaveAnalysis, startCall: startCall, endCall: endCall,
        ...
        startNote: startNote, sendNote: sendNote, cancelNote: cancelNote, mute: function () { S.muted = !S.muted; },
    ...
    setShell: function (v) { root.setAttribute('data-shell', v === 'B' ? 'B' : 'A'); lastMix = null; render(); return shell(); },
  };
  ...
  function syncToggles() {
    Array.prototype.forEach.call(document.querySelectorAll('[data-toggle]'), function (b) {
      ...
      var cur = g === 'who' ? S.whoPref : g === 'shell' ? shell() : (root.getAttribute('data-appearance-source') === 'system' ? 'system' : appearance());
  ...
  Array.prototype.forEach.call(document.querySelectorAll('[data-toggle]'), function (b) {
    b.addEventListener('click', function () {
      ...
      else if (g === 'shell') window.__G31.setShell(v);
      ...
      else window.__G31.setAppearance(v);
      syncToggles();
    });
  });

  reset(); render();
  window.__G31.ready = document.fonts.load('400 17px Estedad').then(function () { return document.fonts.load('500 14px Estedad'); })
