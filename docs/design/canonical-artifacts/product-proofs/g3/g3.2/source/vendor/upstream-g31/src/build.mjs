...
#phone .dk{${vars('dark')}}
#phone{--W:390px;--H:844px;--top:47px;--hdr:48px;--rail:56px;--home:34px;--comp:64px;--tl:88px;--callH:0px;--band-h:0px;
...
#falloff-bottom{position:absolute;bottom:0;inset-inline:0;z-index:2;pointer-events:none;height:calc(var(--home) + var(--rail) + var(--callH) + var(--band-h) + var(--tl) + 70px);
  background:linear-gradient(0deg,rgba(var(--world-rgb),.96) 0,rgba(var(--world-rgb),.93) calc(var(--home) + var(--rail) + var(--callH) + var(--band-h) + 30px),rgba(var(--world-rgb),.62) calc(var(--home) + var(--rail) + var(--callH) + var(--band-h) + var(--tl)),rgba(var(--world-rgb),0) 100%)}
...
function voiceRowHTML() {
  return `<div class="vn"><button class="play" type="button" data-playing="0">${svg(FUNC_GLYPHS.play, { size: 20, cls: 'g-play' })}${svg(FUNC_GLYPHS.pause, { size: 20, cls: 'g-pause' })}</button>` +
...
<section id="shared" aria-labelledby="s-title">
  ...
  return `<div id="phone" dir="${L.dir}" lang="${L.lang}" data-lang="${lang}" data-appearance="dark" data-shell="A" data-place="conversation" data-call="none" data-composer="idle" data-mode="FOLLOW_LIVE" aria-label="${esc(L.product.text)}">
...
<title>QANDEEL — I-08B3.1-G3.1 integrated end-to-end Product proof</title>
...
<div id="mount"></div>
${harness()}
<script>window.__G31DATA=${JSON.stringify(data)};</script>
<script>
(function(){var p=new URLSearchParams(location.search);
 ...
 /* Q-LIGHT-SHELL is OPEN. ?shell=A|B is a HARNESS fixture only: A = the surrounding shell goes dark with the Analysis
    (G2.2's proof answer), B = the surrounding shell keeps following the system. Neither is a Product decision. */
 ph.setAttribute('data-shell',p.get('shell')==='B'?'B':'A');
 window.__G31BOOT={lang:lang};
})();
</script>
<script>${BIDI}</script>
<script>${APP}</script>
...
<h1>QANDEEL · I-08B3.1-G3.1 — integrated end-to-end Product coherence proof</h1>
<p>One Product state machine. Everything in the phone is Product; everything here simulates the device, the other human, or jumps to a journey start. World: <code id="sha-state">verifying…</code></p>
<h2>Journeys (start points)</h2>
<div class="btns">${b('CONV', 'J1 Conversation')}${b('P1', 'Analysis · FAR')}${b('P3', 'NEAR · inspecting')}${b('P4', 'J3 PINNED(14)')}${b('CALL_ANALYSIS', 'J2 in a call · Analysis')}${b('CALL_CONV', 'J2 same call · Conversation')}${b('REPLAY', 'J4 Replay entry')}${b('M1_CUE', 'J5 Matching attention')}</div>
...
<h2 class="open">Q-LIGHT-SHELL — OPEN (harness fixture, not a Product decision)</h2>
<div class="btns">${t('shell', 'A', 'A · shell goes dark with the Analysis')}${t('shell', 'B', 'B · shell keeps following the system')}</div>
<p>Only visible under system Light. The Living Analysis World itself is identical under A and B and under Dark / Light.</p>
<h2>Truth</h2>
<pre id="truth-view">—</pre>
<pre id="act-log">—</pre>
<p>Parameters: <code>?lang=en</code> · <code>?appearance=light</code> · <code>?shell=B</code> · <code>?rm=1</code> · <code>?w=320&amp;h=568</code> · <code>?state=P4</code>. No voice runtime exists: call audio follows G1.2's fixed script and nothing is recorded or sent. The Replay player is not built.</p>
</aside>`;
}
