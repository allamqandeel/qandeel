...
| `hbs.avoidance` | Avoidance | Behavior / HBS / HIF Part 8 v0.1 | STATE | SITUATION, GOAL | Context-bound representation, not a trait |
| `hbs.consistency` | Consistency | Behavior / HBS / HIF Part 8 v0.1 | STATE | SITUATION, GOAL | Context-bound representation, not a trait |
| `hbs.initiative` | Initiative | Behavior / HBS / HIF Part 8 v0.1 | STATE | SITUATION, GOAL | Context-bound representation, not a trait |
| `hbs.reflection` | Reflection | Behavior / HBS / HIF Part 8 v0.1 | STATE | SITUATION, CONVERSATION_SESSION | Context-bound representation, not a trait |
| `hrs.relationship-trust` | Trust | Relationship / HRS / HIF Part 8 v0.1 | STATE | RELATIONSHIP | Not SENA, provider, epistemic, or global trust |
| `hrs.communication` | Communication | Relationship / HRS / HIF Part 8 v0.1 | STATE | RELATIONSHIP | Relationship-bound |
| `hrs.repair` | Repair | Relationship / HRS / HIF Part 8 v0.1 | STATE | RELATIONSHIP | Relationship-bound |
| `hrs.emotional-safety` | Emotional Safety | Relationship / HRS / HIF Part 8 v0.1 | STATE | RELATIONSHIP | Relationship-bound |
| `hgs.self-awareness` | Self-awareness | Growth / HGS / HIF Part 8 v0.1 | CAPABILITY | GOAL, SITUATION | Capability tag, not a trajectory |
| `hgs.resilience` | Resilience | Growth / HGS / HIF Part 8 v0.1 | CAPABILITY | GOAL, SITUATION | Capability tag, not a trajectory |
| `hgs.purpose-alignment` | Purpose Alignment | Growth / HGS / HIF Part 8 v0.1 | ALIGNMENT | GOAL | Goal-bound alignment |
| `hgs.habit-strength` | Habit Strength | Growth / HGS / HIF Part 8 v0.1 | CAPABILITY | GOAL, SITUATION | Capability tag, not a trajectory |

## Mapping rationale and provenance

HSE metrics use `STATE`, never `TRAIT`. Foundation has no `BEHAVIOR` or `RELATIONSHIP` semantic tag, so HBS and HRS are represented conservatively as context-bound `STATE`; the tag is a runtime storage classification, not a new construct definition. HGS uses `CAPABILITY` only where the construct names a capacity and `ALIGNMENT` for Purpose Alignment. No metric is global. Exact context kind and identity are required on every read/write and there is no cross-context fallback.
