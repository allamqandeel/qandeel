...
- [Historical Coverage Completion + Layer A Projection + Layer B Disclosure v1](historical-projection-v1.md) — T-03C (one Architecture-sized task), migration 0072: SP-native availability / validity capture for every v1 exposed family (per-user World Semantic Clock, LEGACY UNCOVERED SESSION vs COVERED at creation, the SP(1) baseline cut under the world-clock lock, server-owned Session association through the durable post-response execution, typed per-family history with deterministic v5 identities, Thread ↔ Reading and Formal Question ↔ Turn appearances, R-C5 expiry mapped into Session Position space, R-C2 preservation, R-C3 tracked legacy attach paths), the owner-scoped Layer-A projection `K(TC) = TemporalProject(W, TC)` and the server-side Layer-B disclosure `V = Disclose(K(TC), semanticDepth, inspectionContext)` with the passive typed client seam; Stage 6.6 v3 matrix reconciled 31 FULL AFTER BUILD / 4 NOT EXPOSED / 0 BLOCKER; no Return-to-Live-Focus, no visual UI
