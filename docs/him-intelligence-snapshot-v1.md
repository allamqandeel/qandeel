...
The server-owned slots are: SITUATION — Stress, Motivation, Self-Confidence, Attention; CONVERSATION_SESSION — Stress, Energy, Attention; DECISION — Self-Confidence, Attention; GOAL — Motivation. Their order follows canonical HSE order and has no ranking meaning. GLOBAL, RELATIONSHIP, Energy-in-SITUATION, and all twelve HBS/HRS/HGS metrics are excluded (the four HBS metrics, `hrs.relationship-trust`, `hrs.communication`, and `hrs.repair` are calibrated but deliberately NOT Snapshot v1 eligible — relationship metrics are sensitive and must not influence advice before a deliberately reviewed HRS consumption contract exists; the other five remain uncalibrated).
